...
export default async function ShipmentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const approvedCountries = user.isAdmin || titeViewOnly ? undefined : user.approvedCountries;

  const [shipment, documents, activityLog, notificationContacts, notificationLog, stats] =
    await Promise.all([
  ...
  const urgentCount =
    (stats?.overdue_count ?? 0) + (stats?.urgent_count ?? 0) + (stats?.action_count ?? 0);
