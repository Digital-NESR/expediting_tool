...
function getCountryCoords(name: string): [number, number] | null {
  ...
  const canonical =
    ALIASES[normalised] ?? Object.keys(COUNTRY_COORDS).find((k) => k.toLowerCase() === normalised);
...
function arcPoints(from: [number, number], to: [number, number], segments = 24): L.LatLngTuple[] {
