...
const INPUT =
  'rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-[#307c4c] focus:ring-2 focus:ring-[#307c4c]/25';
...
export default function LaptopRequestsClient({ data }: { data: LaptopRequestListData | null }) {
  ...
  const pendingCount = requests.filter((r) => {
    ...
    const actions = getLaptopAvailableActions(
      ownsCurrentStep,
      r.status,
      laptopHasAssignedUnit(r),
      laptopIsProcureNewFlow(r),
    );
    return (
      actions.canApprove ||
      actions.canReject ||
  ...
  const requestTypes = Array.from(
    new Set(requests.map((r) => r.request_type).filter(Boolean)),
  ) as string[];
          ...
          <div className="flex flex-col gap-3 border-b border-slate-100 p-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-0.5 text-sm text-slate-500">
                Laptop / desktop procurement and purchase exception requests.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <input
                ...
                placeholder="Search ref, requester, model..."
                  ...
                  paged.map((r) => (
                    <tr key={r.id} className="transition-colors hover:bg-white">
                      ...
                      <td className="px-5 py-4 text-right align-top">
                        <Link
                          href={`/laptop-procurement/requests/${r.id}`}
                          className="inline-flex min-w-[4.5rem] items-center justify-center whitespace-nowrap rounded-lg bg-[#307c4c] px-3.5 py-1.5 text-xs font-bold text-white shadow-sm transition hover:-translate-y-px"
                        >
                          View
                        </Link>
                      </td>
          ...
          {filtered.length > 0 && (
            <div className="flex items-center justify-between border-t border-slate-100 px-5 py-3">
              <p className="text-xs text-slate-500/80">
                ...
                {Math.min((currentPage + 1) * REQUESTS_PAGE_SIZE, filtered.length)} of{' '}
