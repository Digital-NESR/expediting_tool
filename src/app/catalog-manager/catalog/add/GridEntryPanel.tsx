...
const cellCls =
  'w-full rounded-md border border-slate-200 bg-white px-2 py-1.5 text-[12.5px] outline-none focus:border-[#307c4c] focus:ring-1 focus:ring-[#307c4c]/30';
...
export default function GridEntryPanel({
  ...
  const isBlankRow = (r: GridRow) =>
    !r.supplier.trim() && !r.code.trim() && !r.description.trim() && !r.price.trim();
    ...
    return (
      <div className="space-y-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-base font-bold text-slate-900">Grid submitted</p>
            <p className="text-xs text-slate-400">{filledRows.length} rows processed</p>
          </div>
        </div>
        <div className="grid max-w-md grid-cols-3 gap-3">
          ...
          <div className="rounded-xl border border-slate-100 bg-slate-50 px-4 py-3">
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Skipped</p>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Icon name="plus" className="h-4 w-4" /> New grid
          </button>
          <Link
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-slate-500">
        Type rows straight into the grid — one catalog entry per row. Rates over the threshold go to
        Pending Approval, the rest activate. Leave a row blank to skip it.
      </p>
      {error && (
          ...
          <tbody>
            {rows.map((r, idx) => {
              ...
              return (
                ...
                >
                  <td className="px-2 py-1.5 text-center text-[11px] text-slate-400 tabular-nums">
                    {idx + 1}
                  </td>
                  <td className="px-1.5 py-1.5 min-w-[160px]">
                    <input
                      className={cellCls}
                      value={r.supplier}
                      onChange={(e) => setCell(r.key, { supplier: e.target.value })}
                  ...
                  <td className="px-1.5 py-1.5 min-w-[88px]">
                    <select
                      className={cellCls}
                      value={r.country}
                      onChange={(e) => setCell(r.key, { country: e.target.value })}
                    >
                      {countries.map((c) => (
                  ...
                  <td className="px-1.5 py-1.5 min-w-[170px]">
                    <select
                      className={cellCls}
                      value={r.category}
                      onChange={(e) => setCell(r.key, { category: e.target.value })}
                    >
                  ...
                  <td className="px-1.5 py-1.5 min-w-[150px]">
                    <select
                      className={cellCls}
                      value={r.subcategory}
                      onChange={(e) => setCell(r.key, { subcategory: e.target.value })}
                      disabled={!cat}
                    >
                  ...
                  <td className="px-1.5 py-1.5 min-w-[200px]">
                    <input
                      list="gridServiceOpts"
                      className={cellCls}
                      value={r.description}
                      onChange={(e) => setCell(r.key, { description: e.target.value })}
                  ...
                  <td className="px-1.5 py-1.5 min-w-[110px]">
                    <select
                      className={cellCls}
                      value={r.uom}
                      onChange={(e) => setCell(r.key, { uom: e.target.value })}
                    >
                  ...
                  <td className="px-1.5 py-1.5 min-w-[100px]">
                    <input
                      ...
                      className={`${cellCls} font-mono`}
                      value={r.price}
                      onChange={(e) => setCell(r.key, { price: e.target.value })}
                      placeholder="0.00"
                    />
                  </td>
                  <td className="px-1.5 py-1.5 min-w-[78px]">
                    <select
                      className={cellCls}
                      value={r.currency}
                      onChange={(e) => setCell(r.key, { currency: e.target.value })}
                    >
                      {currencies.map((c) => (
                        <option key={c.code} value={c.code}>
                          {c.code}
                  ...
                  <td className="px-1.5 py-1.5 min-w-[140px]">
                    <input
                      ...
                      className={cellCls}
                      value={r.effective}
                      onChange={(e) => setCell(r.key, { effective: e.target.value })}
                    />
                  </td>
                  <td className="px-1.5 py-1.5 min-w-[140px]">
                    <input
                      ...
                      className={cellCls}
                      value={r.expiry}
                      onChange={(e) => setCell(r.key, { expiry: e.target.value })}
                    />
                  </td>
                  <td className="px-1.5 py-1.5 min-w-[110px]">
                    <select
                      className={cellCls}
                      value={r.incoterms}
                      onChange={(e) => setCell(r.key, { incoterms: e.target.value })}
                    >
                      <option value="">—</option>
                      {INCOTERMS.map((ic) => (
                  ...
                  <td className="px-1.5 py-1.5 min-w-[150px]">
                    <input
                      className={cellCls}
                      value={r.incoterms_location}
                      onChange={(e) => setCell(r.key, { incoterms_location: e.target.value })}
                  ...
                  <td className="px-1.5 py-1.5 min-w-[90px]">
                    <input
                      ...
                      className={`${cellCls} font-mono`}
                      value={r.lead_time}
                      onChange={(e) => setCell(r.key, { lead_time: e.target.value })}
                      placeholder="days"
                    />
                  </td>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          <span className="text-[12.5px] text-slate-500">
            {filledRows.length} {filledRows.length === 1 ? 'row' : 'rows'} ready
          </span>
          <button
            onClick={submit}
            disabled={phase === 'running' || filledRows.length === 0}
            className="inline-flex items-center gap-2 rounded-lg bg-[#307c4c] px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-[#2b6f44] disabled:opacity-50"
          >
            <Icon name="check" className="h-4 w-4" />{' '}
            {phase === 'running'
              ? 'Submitting…'
              : `Submit ${filledRows.length} ${filledRows.length === 1 ? 'entry' : 'entries'}`}
          </button>
        </div>
      </div>

      <datalist id="gridServiceOpts">
        {services.map((s) => (
