    ...
    <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <label>
          ...
          <select
            value={role}
            onChange={(e) => setRole(e.target.value as CatalogRole)}
      ...
      {role === 'Approver' && (
        <p className="mt-2 text-[11px] text-amber-700">
          Approver alone doesn&apos;t scope them to a country — also assign them in Catalog Repo →
          Admin → Country approvers.
        </p>
      )}
      <div className="mt-3 flex items-center gap-2">
        <button
          type="button"
          disabled={loading}
          onClick={() => onConfirm(role, country)}
...
export function CatalogAccessApprovalsClient({
  ...
  function handleApprove(row: CatalogAccessRequestRow, role: CatalogRole, countryCode: string) {
    ...
    startTransition(async () => {
      ...
      const result = await approveCatalogAccessRequest({
        userEmail: row.user_email,
        approvedRole: role,
        countryCode: countryCode || null,
      });
      if (!result.success) {
        setActionError(result.error ?? 'Failed to approve access.');
  ...
  function handleReject(email: string) {
    ...
    startTransition(async () => {
      await rejectCatalogAccessRequest(email);
      await refreshData();
      setProcessingEmail(null);
    });
  }
  function handleRevoke(email: string) {
    setProcessingEmail(email);
    startTransition(async () => {
      await revokeCatalogAccessRequest(email);
      await refreshData();
      setProcessingEmail(null);
    });
  }
  function handleDelete(email: string) {
    setProcessingEmail(email);
    startTransition(async () => {
      await deleteCatalogAccessRequest(email);
      await refreshData();
      setProcessingEmail(null);
    });
  }
                ...
                return (
                  <tr key={row.user_email} className="align-top hover:bg-[#307c4c]/5">
                    <td className="px-4 py-3">
                      ...
                      {row.reason && (
                        <p
                          className="mt-1 max-w-xs truncate text-xs italic text-slate-400"
                          title={row.reason}
                        >
                    ...
                    <td className="px-4 py-3 text-slate-600">
                      {row.country_code || <span className="text-slate-300">—</span>}
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      ...
                      {expanded && (
                        <RoleSelector
                          ...
                          onConfirm={(role, countryCode) => handleApprove(row, role, countryCode)}
                        />
                      )}
                    </td>
                  </tr>
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold tracking-tight text-slate-900">
            Catalog Repo Access Approvals
          </h2>
          <p className="mt-0.5 text-[12px] text-gray-400">
            New users default to read-only Viewer automatically — this queue is only for requested
            upgrades.
            {lastRefreshed &&
...
export function CatalogAdminPanelClient() {
  ...
  useEffect(() => {
    getCatalogAdminSummary().then(setData);
  }, []);

  if (!data)
    return (
      <div className="py-16 text-center text-sm text-slate-400">
        Loading Catalog Repo master data…
      </div>
    );
    ...
    <div>
      <div className="mb-5">
        <h2 className="mb-1 text-lg font-bold tracking-tight text-slate-900">
          Catalog Repo · Admin Panel
        </h2>
        <p className="text-[13px] text-slate-500">
          Read-only master-data snapshot. Edit values inside Catalog Repo → Administration.
        </p>
      </div>
...
export function CatalogSyncHealthClient() {
  ...
  if (loading || !data)
    return (
      <div className="py-16 text-center text-sm text-slate-400">Checking PIR sync health…</div>
    );
  ...
  return (
    <div>
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="mb-1 text-lg font-bold tracking-tight text-slate-900">
            PIR / Inventory · Sync Health
          </h2>
          <p className="text-[13px] text-slate-500">
            Status of the nightly n8n load from Power BI into pir_catalog.
          </p>
        </div>
        <button
          type="button"
          onClick={refresh}
          className="rounded-md border border-slate-200 px-3.5 py-2 text-[13px] font-medium text-slate-600 hover:bg-slate-50"
        >
          Refresh
        </button>
      </div>
        ...
        <div className="mb-5 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
          ...
          <div>
            <p className="text-[13px] font-bold text-amber-800">
              {data.lastSyncedAt
                ? `No sync in ${data.hoursSinceSync?.toFixed(1)} hours`
                : 'No successful sync recorded yet'}
            </p>
            <p className="mt-0.5 text-[12px] text-amber-700">
              The nightly n8n workflow may have failed its validation guard (e.g. Power BI
              mid-refresh) or isn&apos;t running. Check the n8n execution log.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Kpi
          label="Last synced"
          value={data.hoursSinceSync != null ? `${data.hoursSinceSync.toFixed(1)}h ago` : 'Never'}
          tone={data.isStale ? 'warn' : 'good'}
          sub={data.lastSyncedAt ? formatDate(data.lastSyncedAt) : undefined}
        />
        <Kpi
          label="Description coverage"
          value={`${data.descriptionCoveragePct}%`}
          sub={`${data.withDescription.toLocaleString()} of ${data.total.toLocaleString()}`}
          tone={
            data.descriptionCoveragePct < 20
              ? 'bad'
              : data.descriptionCoveragePct < 50
                ? 'warn'
                : 'good'
          }
        />
        <Kpi
          label="Status"
          value={data.isStale ? 'Stale' : 'Healthy'}
          tone={data.isStale ? 'warn' : 'good'}
        />
      </div>
      <p className="mt-4 text-[11.5px] text-slate-400">
        Some PIR materials genuinely have no description (never referenced on a purchase order in
        SUPPLYCHAIN, the only description source) — coverage below 100% is expected. A sudden drop,
        or &quot;stale&quot; above, usually means the SUPPLYCHAIN dataset was mid-refresh when the
        sync ran.
      </p>
    </div>
  );
