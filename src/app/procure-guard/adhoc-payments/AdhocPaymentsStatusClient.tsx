...
export default function AdhocPaymentsStatusClient({
  data,
}: {
  data: ProcureGuardRequestListData<AdhocPaymentRequest> | null;
}) {
  ...
  return (
    <div className="min-h-[100dvh] bg-slate-50 font-sans text-slate-900">
      ...
      <main className="max-w-[1220px] mx-auto px-4 sm:px-6 py-4 space-y-4">
        <ProcureGuardHero
          title="Adhoc PO Status"
          subtitle="Track and review adhoc purchase order requests through the approval workflow."
        />
          ...
          <div className="p-4 border-b border-slate-100 flex flex-col lg:flex-row gap-3 lg:items-center lg:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                One-off vendor payments, urgent supplier releases, and exception payments.
              </p>
            </div>
                ...
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-5 py-10 text-center text-slate-500">
                      No adhoc payment requests found.
                    </td>
                  </tr>
                ) : (
                  filtered.map((r) => (
                    <tr key={r.id} className="hover:bg-[#307c4c]/5">
                      <td className="px-5 py-4 align-top">
                        <Link
                          href={`/procure-guard/adhoc-payments/${r.id}`}
