...
export default function CatalogAnalyticsClient({
  ...
  const catData = data.byCategory
    .slice(0, 8)
    .map((c) => ({ name: c.name, type: c.type ?? '', count: c.activeCount }));
  const ctyData = data.byCountry.map((c) => ({
    name: `${c.flag ?? ''} ${c.code}`.trim(),
    fullName: c.name,
    count: c.activeCount,
  }));
      ...
      <div className="cm-stagger space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Rate coverage and price trends across{' '}
            {scope === 'ALL'
              ? 'all operating countries'
              : (countries.find((c) => c.code === scope)?.name ?? scope)}
            .
          </p>
        </div>
        ...
        <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard
            ...
            sub={`${data.supplierCount} suppliers`}
            icon="catalog"
          />
          <StatCard
            label="Avg rate change"
            value={
              data.avgRateChangePct == null
                ? '—'
                : `${data.avgRateChangePct >= 0 ? '+' : ''}${data.avgRateChangePct.toFixed(1)}%`
            }
          ...
          <StatCard
            ...
            sub={data.pendingCount ? 'awaiting sign-off' : 'queue clear'}
            icon="approve"
            tone="ink"
          />
          <StatCard
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <Card className="p-5">
            <CardHeader
              className="mb-4"
              title="Active rates by category"
              sub="Top categories by active rate count."
            />
            {catData.length === 0 ? (
              <EmptyState icon="layers" title="No active rates" />
            ) : (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Bar dataKey="count" radius={[0, 6, 6, 0]} barSize={18}>
                      {catData.map((_, i) => (
                        <Cell key={i} fill={i === 0 ? GREEN : i < 3 ? GREEN_LIGHT : GREEN_PALE} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
          ...
          <Card className="p-5">
            <CardHeader
              className="mb-4"
              title="Active rates by country"
              sub="Coverage of the catalog per operating country."
            />
            {ctyData.length === 0 ? (
              <EmptyState icon="globe" title="No active rates" />
            ) : (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={ctyData} margin={{ top: 8, right: 8, bottom: 0, left: -18 }}>
                    ...
                    <Bar dataKey="count" fill={GREEN} radius={[6, 6, 0, 0]} barSize={34}>
                      {ctyData.map((_, i) => (
                        <Cell key={i} fill={i % 2 ? GREEN_LIGHT : GREEN} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
        ...
        <Card className="p-5">
          <CardHeader
            className="mb-2"
            title="Catalog status mix"
            sub={`${totalStatus.toLocaleString()} entries across all statuses.`}
          />
          {statusData.length === 0 ? (
            <EmptyState icon="chart" title="No entries yet" />
          ) : (
              ...
              <div className="grid flex-1 grid-cols-1 gap-2 sm:grid-cols-2">
                {statusData.map((s) => (
                  ...
                  >
                    ...
                    <span className="w-11 text-right text-[11px] tabular-nums text-slate-400">
                      {totalStatus ? Math.round((s.value / totalStatus) * 100) : 0}%
                    </span>
                  </div>
                ))}
              </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3.5">
            ...
            <span className="text-[11px] text-slate-400">
              first vs. current price · re-priced entries
            </span>
          </div>
          {data.topMovers.length === 0 ? (
            <EmptyState
              icon="trend"
              title="No re-priced entries yet"
              sub="Entries with more than one rate version will show their price trend here."
            />
          ) : (
                ...
                <tbody>
                  {data.topMovers.map((m) => {
                    ...
                    return (
                      <tr
                        key={m.id}
                        className="border-b border-slate-100 transition-colors hover:bg-[#307c4c]/5"
                      >
                        <td className="px-4 py-3">
                          <Link
                            href={`/catalog-manager/catalog/${m.id}`}
                            className="font-mono text-[12px] text-[#1d4f31] hover:underline"
                          >
                            {m.code}
                          </Link>
                        </td>
                        <td className="max-w-[280px] px-4 py-3">
                          ...
                          <div className="truncate text-[11px] text-slate-400">
                            {m.commodity || m.item_name} · {m.country_code}
                          </div>
                        </td>
                        <td className="whitespace-nowrap px-4 py-3 text-right font-mono text-slate-500">
                          {m.firstPrice.toLocaleString()} {m.currency_code}
                        </td>
                        <td className="whitespace-nowrap px-4 py-3 text-right font-mono font-semibold text-slate-900">
                          {m.currentPrice.toLocaleString()} {m.currency_code}
                        </td>
                        <td className="whitespace-nowrap px-4 py-3 text-right">
                          <span
                            className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 font-semibold ${up ? 'bg-amber-50 text-amber-600' : 'bg-[#307c4c]/10 text-[#307c4c]'}`}
                          >
                            <Icon
                              name="trend"
                              className={`h-3.5 w-3.5 ${up ? '' : '-scale-y-100'}`}
                            />
                            {up ? '+' : ''}
