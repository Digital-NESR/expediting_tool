/**
 * The NextAuth configuration: providers, callbacks, and the per-user access resolver.
 *
 * This lived inside `src/app/api/auth/[...nextauth]/route.ts` and was re-exported by
 * `src/lib/auth.ts`, which inverted the dependency — twenty-eight modules reached a route
 * handler for a value that is not a handler. Next's route type-guard is designed to reject a
 * route file that exports anything other than GET/POST/config, so the arrangement was also one
 * type-check away from failing. The route now imports from here instead.
 *
 * Note the file sits at `src/lib/auth/options.ts`, next to `src/lib/auth.ts` rather than
 * replacing it: `@/lib/auth` still resolves to the file, `@/lib/auth/options` to this module.
 * Same deliberate arrangement as `@/lib/db` and `@/lib/db/pool`.
 */
    ...
    async jwt({ token, account, trigger }) {
      if (account?.provider === 'azure-ad' && account.access_token) {
        ...
        /* Fetch the profile photo from Microsoft Graph and store the BYTES
           ...
           the proxy each time. The token now carries one boolean and the
      ...
      if (email) {
        ...
        try {
          ...
          /* The Learning Hub used to resolve an access status here, one query per uncached
             session across every signed-in employee. Nothing read it: the hub is open to
             everyone, and the proxy has no gate for it. The query and the status are gone. */
