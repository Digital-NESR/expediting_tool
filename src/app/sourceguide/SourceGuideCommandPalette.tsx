...
type Flat = {
  kind: 'commodity' | 'supplier' | 'category' | 'country';
  label: string;
  sub?: string;
  href: string;
  tone?: string | null;
...
export default function SourceGuideCommandPalette({
  isOpen,
  onClose,
}: {
  isOpen: boolean;
  ...
  const flat: Flat[] = useMemo(() => {
    ...
    res.commodities.forEach((c) =>
      f.push({
        kind: 'commodity',
        label: c.name,
        sub: [c.category, c.subCategory].filter(Boolean).join(' · '),
        href: `/sourceguide/commodity/${c.id}`,
      }),
    );
    res.suppliers.forEach((s) =>
      f.push({
        ...
        label: s.name,
        sub: `Vendor ${s.code}`,
        href: `/sourceguide/suppliers/${encodeURIComponent(s.code)}`,
      }),
    );
    res.categories.forEach((c) =>
      f.push({
        ...
        label: c.name,
        sub: 'Category',
        href: `/sourceguide/search?cat=${encodeURIComponent(c.id)}`,
      }),
    );
    res.countries.forEach((c) =>
      f.push({
        ...
        label: c.name,
        sub: 'Country guide',
        href: `/sourceguide/country/${c.code}`,
        tone: c.tone,
      }),
    );
    return f;
        ...
        <div ref={listRef} className="max-h-[55vh] overflow-y-auto py-2">
          {!q.trim() && (
            <div className="px-4 py-8 text-center text-[13px] text-slate-400">
              Start typing to search across SourceGuide.
            </div>
          )}
          ...
          {flat.map((item, i) => {
            ...
            return (
              <Fragment key={i}>
                {showHeader && (
                  <div className="px-4 pb-1 pt-2 font-mono text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400">
                    {titleFor(item.kind)}
                  </div>
                )}
                <button
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    {item.sub && (
                      <span className="block truncate text-[12px] text-slate-400">{item.sub}</span>
                    )}
                  </span>
