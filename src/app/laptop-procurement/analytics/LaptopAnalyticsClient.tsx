          ...
          {data.map((d) => (
            <div key={d.label}>
              ...
              <div className="mt-1 h-2 overflow-hidden rounded-full bg-slate-100">
                <div
                  className="h-full rounded-full bg-[#307c4c]"
...
export default function LaptopAnalyticsClient({
  data,
  embedded = false,
}: {
    ...
    <div className="space-y-5">
      ...
      <section className={`${GLASS} p-5`}>
        ...
        <p className="mt-2 text-xs text-slate-500">
          Based on the original requested date (from the Power BI export).
        </p>
      </section>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                ...
                outerRadius={100}
                label={(entry) => {
                  const e = entry as unknown as { label?: string; count?: number };
                  return `${e.label ?? ''}: ${e.count ?? 0}`;
                }}
                labelLine={false}
              >
                {data.device_breakdown.map((entry, i) => (
