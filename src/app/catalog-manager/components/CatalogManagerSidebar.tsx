...
export default function CatalogManagerSidebar({
  ...
  const nav: NavItem[] = (
    [
      ...
      {
        href: '/catalog-manager/analytics',
        label: 'Analytics',
        icon: 'chart',
        section: 'Workspace',
      },
      {
        href: '/catalog-manager/request-access',
      ...
      {() => (
        <>
          <div className="relative flex h-16 shrink-0 items-center gap-3 overflow-hidden bg-gradient-to-br from-[#307c4c] via-[#2b6f44] to-[#1d4f31] px-5 text-white">
            ...
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white p-1.5 text-[#307c4c] shadow-sm">
            ...
            <div className="ml-auto flex items-center gap-1">
              ...
              <button
                ...
                aria-label="Close menu"
                onClick={onClose}
                className={`rounded-lg p-2 text-white/70 transition-colors hover:bg-white/15 hover:text-white ${pinned ? 'md:hidden' : ''}`}
              >
            ...
            {sections.map((section) => {
              ...
              return (
                <div key={section} className="mt-2">
                  <div className="px-3 pb-1.5 pt-2 text-[10px] font-bold uppercase tracking-wider text-slate-400">
                    {section}
                  </div>
                  <div className="space-y-0.5">
                    {items.map((item) => {
                      ...
                      return (
                        ...
                        >
                          {active && (
                            <span className="absolute left-0 top-1.5 bottom-1.5 w-1 rounded-r-md bg-[#307c4c]" />
                          )}
                          <Icon
                            name={item.icon}
                            className={`h-[18px] w-[18px] transition-colors ${active ? 'text-[#307c4c]' : 'text-slate-400 group-hover:text-slate-600'}`}
                          />
          ...
          <div className="border-t border-slate-200/80 p-3">
            <div className="flex items-center gap-3 rounded-xl border border-slate-200/80 bg-slate-50/60 p-2.5">
              {image ? (
                ...
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-[#307c4c] to-[#2b6f44] text-xs font-semibold text-white ring-2 ring-white">
                  {initials}
                </div>
              )}
