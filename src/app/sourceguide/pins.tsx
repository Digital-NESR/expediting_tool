...
export function recordView(item: PinItem) {
  ...
  const next = [item, ...read(RECENT_KEY).filter((x) => idOf(x.kind, x.key) !== id)].slice(
    0,
    MAX_RECENT,
  );
...
export function toggleBookmark(item: PinItem): boolean {
  ...
  write(
    BOOK_KEY,
    exists ? cur.filter((x) => idOf(x.kind, x.key) !== id) : [item, ...cur].slice(0, MAX_BOOK),
  );
  return !exists;
}
...
export function BookmarkButton({ item, className = '' }: { item: PinItem; className?: string }) {
  ...
  return (
    <button
      ...
      style={
        on
          ? { borderColor: SG_BRAND, background: '#eaf4ef', color: '#1f5d3a' }
          : { borderColor: '#D1D3D4', background: '#fff', color: '#58595B' }
      }
    >
      <Star
        className="h-3.5 w-3.5"
        fill={on ? SG_BRAND : 'none'}
        style={{ color: on ? SG_BRAND : undefined }}
      />
