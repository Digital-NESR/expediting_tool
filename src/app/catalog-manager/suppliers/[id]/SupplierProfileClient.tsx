      ...
      <div className="cm-stagger mx-auto max-w-4xl space-y-5">
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3.5">
            <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-[#eaf4ef] to-white text-[#1d4f31] ring-1 ring-inset ring-[#307c4c]/15 shadow-sm">
                ...
                {profile.manager && (
                  <>
                    ...
                    <span className="inline-flex items-center gap-1.5">
                      <Avatar name={profile.manager} size={18} /> {profile.manager}
                    </span>
                  </>
                )}
              </div>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3">
            ...
            <div className="rounded-xl border border-slate-100 bg-slate-50 px-3.5 py-3">
              ...
              <p className="mt-0.5 text-xl font-bold tabular-nums text-[#307c4c]">
                {profile.activeCount}
              </p>
            </div>
            <div className="rounded-xl border border-slate-100 bg-slate-50 px-3.5 py-3">
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
          ...
          {profile.contactEmails.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {profile.contactEmails.map((email) => (
                <a
                  key={email}
                  href={`mailto:${email}`}
                  className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 text-[12.5px] font-medium text-[#1d4f31] hover:border-[#307c4c]/30 hover:bg-[#307c4c]/5"
                >
            ...
            <p className="text-[12.5px] text-slate-400">
              No contact emails on file in the SAP supplier master.
            </p>
          )}
        </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
          <div className="border-b border-slate-100 px-5 py-3.5">
            <h2 className="text-[13.5px] font-bold text-slate-900">Rate card</h2>
          </div>
              ...
              <tbody>
                {profile.entries.map((e) => (
                  <tr
                    ...
                    className="cursor-pointer border-b border-slate-100 transition-colors hover:bg-[#307c4c]/5"
                  >
                    ...
                    <td className="max-w-[280px] px-4 py-3">
                      ...
                      <div className="truncate text-[11px] text-slate-400">{e.category_name}</div>
                    </td>
                    ...
                    <td className="whitespace-nowrap px-4 py-3 text-right">
                      ...
                      <span className="text-[10.5px] text-slate-400">{e.currency_code}</span>
                    </td>
                    <td className="px-4 py-3">
                      <StatusPill status={e.status} sm />
                    </td>
                    <td className="whitespace-nowrap px-4 py-3 text-[12.5px] text-slate-500">
                      {e.expiry_date ? fmtDateNice(e.expiry_date) : '—'}
                    </td>
                  </tr>
                ))}
              </tbody>
