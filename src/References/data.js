// Excel epoch helper (Excel serial date 45000 ≈ 2023-03-15)
const xd = (serial) => {
...
// Real records lifted from the Excel + invented to round out states.
// Status logic is computed downstream from expiry/extended.
const RAW_SHIPMENTS = [
  ...
  // Invented to surface every alert state cleanly relative to today (May 1, 2026 ≈ 46143)
...
const computeStatus = (s) => {
  if (s.status === 'Closed') return { ...s, daysToExpiry: null, alert: 'closed' };
  const eff = s.extended || s.expiry;
  const daysToExpiry = Math.floor((eff - today) / (1000 * 60 * 60 * 24));
  let alert;
  ...
  const status = daysToExpiry < 0 ? 'Overdue'
              : daysToExpiry <= 14 ? 'At risk'
              : 'Active';
  return { ...s, daysToExpiry, alert, status };
};

const SHIPMENTS = RAW_SHIPMENTS.map(computeStatus);
...
SHIPMENTS.forEach(s => {
  ...
  TIMELINE_BY_ID[s.id] = events.sort((a, b) => a.at - b.at);
});

// Country coordinates for map view (approximate)
const COUNTRY_COORDS = {
  'KSA': { x: 575, y: 305, label: 'Saudi Arabia' },
  'UAE': { x: 615, y: 315, label: 'UAE' },
  'Qatar': { x: 600, y: 305, label: 'Qatar' },
  'Oman': { x: 635, y: 330, label: 'Oman' },
  'India': { x: 720, y: 345, label: 'India' },
  'China': { x: 830, y: 270, label: 'China' },
  'USA': { x: 200, y: 245, label: 'USA' },
  'UK': { x: 470, y: 195, label: 'UK' },
  'Algeria': { x: 480, y: 290, label: 'Algeria' },
  'Iraq': { x: 575, y: 265, label: 'Iraq' },
  '': { x: 0, y: 0, label: 'Unknown' },
};

window.PORTAL_DATA = { SHIPMENTS, DOCS_BY_ID, TIMELINE_BY_ID, COUNTRY_COORDS, today, fmtDate };
