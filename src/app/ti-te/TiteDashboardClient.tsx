...
export default function TiteDashboardClient({
  ...
  const buckets = [
    {
      key: 'overdue',
      label: 'Overdue',
      count: open.filter((s) => s.alert_level === 'overdue').length,
    },
    {
      key: 'urgent',
      label: '≤ 7 days',
      count: open.filter((s) => s.alert_level === 'urgent').length,
    },
    {
      key: 'action',
      label: '8–14 days',
      count: open.filter((s) => s.alert_level === 'action').length,
    },
    {
      key: 'plan',
      label: '15–30 days',
      count: open.filter((s) => s.alert_level === 'plan').length,
    },
    {
      key: 'info',
      label: '31–60 days',
      count: open.filter((s) => s.alert_level === 'info').length,
    },
  ...
  const actionQueue = open.filter((s) => isUrgentAlertLevel(s.alert_level)).slice(0, 8);
        ...
        <div className="flex items-end justify-between gap-4 mb-5">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              {today} · {activeCount} active temporary movements ·{' '}
              {stats.overdue_count + stats.urgent_count} need action this week
            </p>
          </div>
          {!viewOnly && (
            <button
        ...
        {stats.overdue_count > 0 && (
          <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-5 text-sm text-red-800">
            ...
            <span className="flex-1">
              <strong>
                {stats.overdue_count} shipment{stats.overdue_count > 1 ? 's are' : ' is'} past
                re-export deadline.
              </strong>{' '}
              Penalty risk — escalate to legal & customs immediately.
            </span>
            <button
          ...
          {[
            {
              ...
              sub: `${stats.import_count} imports · ${stats.export_count} exports`,
              accent: '#006B0C',
              color: '#1e293b',
            },
            {
            ...
            },
            {
              ...
              value: usdFmt(stats.total_deposit_usd),
              sub: 'Open and extended shipments only',
              accent: '#006B0C',
              color: '#1e293b',
            },
          ].map((kpi, i) => (
                ...
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50 sticky top-0 z-10">
                    {['Shipment', 'Route', 'Movement', 'Deposit', 'Expiry', 'Status'].map((h) => (
                      <th
                        key={h}
                        className="text-left text-[11px] font-semibold uppercase tracking-wider text-slate-400 px-4 py-2.5 whitespace-nowrap bg-slate-50"
                      >
                        {h}
                      </th>
                    ))}
                  ...
                  {actionQueue.map((s) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-2.5 whitespace-nowrap">
                          <span
                            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold whitespace-nowrap ${(s.movement_type || '').toLowerCase().includes('export') ? 'bg-blue-100 text-blue-700 border border-blue-200' : 'bg-cyan-100 text-cyan-700 border border-cyan-200'}`}
                          >
                        ...
                        <td className="px-4 py-2.5">
                          {(() => {
                            ...
                            return (
                              <span
                                className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold whitespace-nowrap ${sb.className}`}
                              >
                                {sb.label}
                              </span>
                            );
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
            ...
            <div className="p-5 space-y-4">
              ...
              {segments.map(([seg, info]) => {
                const pct =
                  stats.total_deposit_usd > 0 ? (info.deposit / stats.total_deposit_usd) * 100 : 0;
                return (
                  <div key={seg}>
                    <div className="flex items-center text-[12.5px] mb-1.5">
                      <span className="font-semibold text-slate-800">{seg}</span>
                      <span className="text-slate-400 text-[11.5px] ml-2">
                        · {info.count} active
                      </span>
                      <div className="flex-1" />
              ...
              <ul className="divide-y divide-slate-50 overflow-y-auto max-h-[320px]">
                {recentActivity.map((row) => (
                  <li key={row.id}>
                    ...
                    >
                      <span
                        className={`w-2.5 h-2.5 rounded-full shrink-0 ${activityDot(row.action)}`}
                      />
