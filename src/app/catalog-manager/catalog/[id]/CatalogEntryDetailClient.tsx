...
function KV({
  ...
  return (
    <div>
      ...
      <div className={`text-[13.5px] font-medium ${amber ? 'text-amber-700' : 'text-slate-900'}`}>
        {children}
      </div>
    </div>
  );
...
export default function CatalogEntryDetailClient({
  ...
  async function onUpload(file: File) {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setDocError('File is too large — max 5 MB.');
      return;
    }
    setUploading(true);
    try {
  ...
  async function removeDoc(docId: number) {
    ...
    setBusy(true);
    try {
      await deleteEntryDocument(docId, entry.id);
      router.refresh();
    } finally {
      setBusy(false);
    }
  ...
  async function doSubmit() {
    setBusy(true);
    try {
      await submitForApproval(entry.id);
      router.refresh();
    } finally {
      setBusy(false);
    }
  ...
  async function doDeactivate() {
    ...
    setBusy(true);
    try {
      await deactivateCatalogEntry(entry.id);
      router.refresh();
    } finally {
      setBusy(false);
    }
      ...
      <div className="cm-stagger mx-auto max-w-3xl space-y-5">
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
          <div className="mb-2 flex flex-wrap items-center gap-2.5">
            <span className="rounded-md border border-slate-200 bg-slate-50 px-2 py-0.5 font-mono text-[12px] text-slate-600">
              {entry.code}
            </span>
          ...
          <Link
            href={`/catalog-manager/suppliers/${entry.supplier_id}`}
            className="group inline-flex items-center gap-2 text-2xl font-bold tracking-tight text-slate-900 hover:text-[#1d4f31]"
          >
            {entry.supplier_name}
          ...
          <div className="mt-2.5 flex flex-wrap gap-2">
            {entry.spend_type && (
              <Chip tone={spendTypeTone(entry.spend_type)}>
            ...
            <Chip>
              ...
              {entry.country_flag} {entry.country_name}
            </Chip>
          </div>
        </div>
        ...
        {homeCountry && entry.country_code !== homeCountry && (
          <div className="flex items-start gap-2.5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-[13px] text-amber-800">
            ...
            <span>
              <span className="font-semibold">Outside your home country.</span> This rate is for{' '}
              <span className="font-semibold">
                {entry.country_flag} {entry.country_name}
              </span>
              , not your home country (<span className="font-semibold">{homeCountry}</span>). Prices
              and approvals are governed by that country&apos;s scope.
            </span>
          </div>
        )}
        ...
        <div className="relative flex items-center justify-between overflow-hidden rounded-2xl border border-[#307c4c]/20 bg-gradient-to-br from-[#eaf4ef] via-[#eaf4ef] to-white p-5 shadow-sm">
          ...
          <div className="relative">
            <div className="mb-1 text-[11px] font-semibold uppercase tracking-wider text-[#1d4f31]">
              Unit price
            </div>
            <div className="flex items-baseline gap-2">
              <span className="font-mono text-4xl font-bold tracking-tight text-slate-900">
                {fmtMoney(entry.unit_price, entry.currency_code)}
              </span>
            ...
            <div className="mt-1 font-mono text-[11.5px] text-slate-500">
              ≈ USD {fmtUsd(entry.usd_equivalent)}
            </div>
          </div>
          <div className="relative text-right">
            ...
            <Chip tone={isApproverTier(entry.tier_label) ? 'amber' : 'green'}>
              <Icon
                name={isApproverTier(entry.tier_label) ? 'approve' : 'check'}
        ...
        {exp && (
          <div className="flex items-center gap-2.5 rounded-lg border border-amber-200 bg-amber-50 px-3.5 py-2.5 text-[13px] font-semibold text-amber-700">
            <Icon name="clock" className="h-4 w-4" /> Rate expires in {dUntil} day
            {dUntil === 1 ? '' : 's'} ({fmtDateNice(entry.expiry_date)}) — consider renewing.
          </div>
        )}
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
          ...
          <p className="text-[15px] font-semibold text-slate-900">
            {entry.commodity || entry.item_name}
          </p>
          {entry.description && (
            <p className="mt-1 text-[13px] leading-relaxed text-slate-500">{entry.description}</p>
          )}

          <SectionTitle className="mb-2.5 mt-5">Spend classification</SectionTitle>
          <div className="grid grid-cols-2 gap-x-5 gap-y-4">
            <KV label="Spend type">{entry.spend_type ?? '—'}</KV>
            <KV label="UNSPSC code">
              <span className="font-mono">{entry.unspsc_code || '—'}</span>
            </KV>
          ...
          <div className="mt-5 grid grid-cols-2 gap-x-5 gap-y-4">
            <KV label="Supplier code">
              <span className="font-mono">{entry.supplier_code}</span>
            </KV>
            <KV label="Supplier manager">
              <span className="inline-flex items-center gap-2">
                <Avatar name={entry.manager} size={20} />
                <span className={entry.manager ? '' : 'text-slate-400'}>
                  {entry.manager || 'Unassigned'}
                </span>
              </span>
            </KV>
            <KV label="Currency">{entry.currency_code}</KV>
            <KV label="Effective date">{fmtDateNice(entry.effective_date)}</KV>
            <KV label="Expiry date" amber={exp}>
              {entry.expiry_date ? fmtDateNice(entry.expiry_date) : 'No expiry'}
            </KV>
            <KV label="Incoterms">
              {entry.incoterms ? (
                <span className="font-mono">{entry.incoterms}</span>
              ) : (
                <span className="text-slate-400">—</span>
              )}
            </KV>
            <KV label="Incoterms location">
              {entry.incoterms_location ?? <span className="text-slate-400">—</span>}
            </KV>
            <KV label="Lead time">
              {entry.lead_time_days != null ? (
                `${entry.lead_time_days} day${entry.lead_time_days === 1 ? '' : 's'}`
              ) : (
                <span className="text-slate-400">—</span>
              )}
            </KV>
          ...
          {entry.notes && (
            <>
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-[13px] leading-relaxed text-slate-600">
                {entry.notes}
              </div>
            </>
          )}
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
          ...
          {entry.documents.length > 0 ? (
            ...
            <div className="flex items-center gap-2 rounded-lg border border-dashed border-slate-300 px-3.5 py-3 text-[12.5px] text-slate-400">
              <Icon name="alert" className="h-4 w-4 text-amber-500" /> No supporting document
              attached.
            </div>
          )}
          ...
          {canCreate && (
            <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-3">
              <select
                ...
                className="rounded-lg border border-slate-300 px-2.5 py-2 text-[12.5px] outline-none focus:border-[#307c4c]"
              >
                {PROOF_TYPES.map((t) => (
              ...
              />
              {docError && (
                <span className="w-full text-[11.5px] font-medium text-red-600">{docError}</span>
              )}
            </div>
          )}
          ...
          {entry.sirion_contract_id && (
            <a
              href={entry.sirion_url ?? '#'}
              target="_blank"
              rel="noreferrer"
              className="mt-3 flex items-center gap-3 rounded-lg border border-[#307c4c]/20 bg-[#307c4c]/10 px-3.5 py-3 hover:border-[#307c4c]/40"
            >
              <span className="flex h-9 w-9 items-center justify-center rounded-lg border border-[#307c4c]/20 bg-white text-[#1d4f31]">
                <Icon name="link" className="h-4 w-4" />
              </span>
              <div className="min-w-0 flex-1">
                <div className="text-[11px] font-semibold uppercase tracking-wider text-[#1d4f31]">
                  Sirion Contract ID
                </div>
              ...
              <span className="inline-flex items-center gap-1.5 text-[12.5px] font-semibold text-[#1d4f31]">
                Open in Sirion <Icon name="external" className="h-4 w-4" />
              </span>
            </a>
          )}
        </div>
        ...
        {/* version history */}
        <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
          <SectionTitle className="mb-3 inline-flex items-center gap-1.5">
            <Icon name="history" className="h-3.5 w-3.5" /> Version history
          </SectionTitle>
          <div className="relative space-y-4 pl-5">
            ...
            {entry.history.map((h, i) => (
              <div key={h.version_no} className="relative">
                <span
                  className={`absolute -left-[14px] top-1 h-3 w-3 rounded-full border-2 ${i === 0 ? 'border-[#307c4c] bg-[#307c4c] ring-4 ring-[#307c4c]/10' : 'border-slate-300 bg-white'}`}
                />
                <div className="text-[13px] font-semibold text-slate-900">
                  v{h.version_no}
                  {i === 0 ? ' · current' : ''}{' '}
                  <span className="ml-1 font-mono font-medium text-[#1d4f31]">
                    {fmtMoney(h.unit_price, h.currency_code)} {h.currency_code}
                  </span>
                </div>
                <div className="text-[12px] text-slate-400">
                  {h.change_reason ? `${h.change_reason} · ` : ''}
                  {h.modified_by} · {fmtDateNice(h.modified_at)}
                </div>
              </div>
            ))}
          </div>
          <div className="mt-4 flex justify-between border-t border-slate-100 pt-3 text-[11.5px] text-slate-400">
            <span>
              Created by {entry.created_by} · {fmtDateNice(entry.created_at)}
            </span>
        ...
        <div className="flex flex-wrap justify-end gap-2.5">
          {canCreate && (entry.status === 'Draft' || entry.status === 'Rejected') && (
            <>
              <Link
                href={`/catalog-manager/catalog/${entry.id}/edit`}
                className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition-all hover:border-[#6aaf8e] active:scale-[0.98]"
              >
              ...
              <button
                onClick={doSubmit}
                disabled={busy}
                className="inline-flex items-center gap-2 rounded-lg bg-[#307c4c] px-4 py-2 text-sm font-semibold text-white shadow-sm shadow-[#307c4c]/25 transition-all hover:bg-[#2b6f44] active:scale-[0.98] disabled:opacity-50"
              >
                <Icon name="arrowRight" className="h-4 w-4" />{' '}
                {entry.status === 'Rejected' ? 'Revise & resubmit' : 'Submit for approval'}
              </button>
            </>
          )}
          {entry.status === 'Pending Approval' && canApproveThis && (
            <>
              <button
                ...
                className="inline-flex items-center gap-2 rounded-lg border border-red-200 bg-white px-4 py-2 text-sm font-semibold text-red-600 shadow-sm transition-all hover:bg-red-50 active:scale-[0.98]"
              >
                <Icon name="x" className="h-4 w-4" /> Reject / revise
              </button>
              <button
          ...
          {entry.status === 'Pending Approval' && canCreate && !canApproveThis && (
            <Chip>
              <Icon name="clock" className="h-3.5 w-3.5" /> Awaiting{' '}
              {entry.approver_name ?? 'approver'}
            </Chip>
          )}
          {entry.status === 'Active' && canCreate && (
            <>
              <button
                ...
                disabled={busy}
                className="inline-flex items-center gap-2 rounded-lg border border-red-200 bg-white px-4 py-2 text-sm font-semibold text-red-600 shadow-sm transition-all hover:bg-red-50 active:scale-[0.98] disabled:opacity-50"
              >
                Deactivate
              </button>
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition-all hover:border-[#6aaf8e] active:scale-[0.98]"
              >
                <Icon name="edit" className="h-4 w-4" /> Edit (new version)
              </Link>
            </>
          )}
        </div>
