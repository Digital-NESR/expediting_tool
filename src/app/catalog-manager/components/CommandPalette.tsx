...
export default function CommandPalette() {
  ...
  const rows = useMemo(
    () => [
      ...
      ...res.suppliers.map((s) => ({
        key: `s-${s.id}`,
        href: `/catalog-manager/suppliers/${s.id}`,
      })),
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener('cm:palette', onTrigger);
    };
  }, []);
          ...
          {q.trim().length < 2 ? (
            <div className="px-4 py-8 text-center">
              <p className="text-[13px] text-slate-400">
                Type at least 2 characters to search the whole catalog.
              </p>
                  ...
                  {res.entries.map((e) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {hot && (
                          <Icon name="chevRight" className="h-3.5 w-3.5 shrink-0 text-[#307c4c]" />
                        )}
                      </button>
                    );
                  })}
