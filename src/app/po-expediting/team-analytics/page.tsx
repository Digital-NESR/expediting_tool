            ...
            <table className="w-full text-left border-collapse" style={{ tableLayout: 'fixed' }}>
              ...
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-[10px] font-semibold text-slate-500 uppercase tracking-wider">
                  ...
                  <th className="py-3 px-4 text-right">Suppliers</th>
                  <th className="py-3 px-4 text-right">PO Lines</th>
...
function SupplierDetailModal({
  ...
  useEffect(() => {
    getAdminSupplierDetail(supplierName)
      .then((data) => {
        setLines(data);
        setExpanded(new Set(data.map((l) => l.po_number)));
      })
      .finally(() => setLoading(false));
  }, [supplierName]);
  ...
  return (
    <DetailModal isOpen title={supplierName} onClose={onClose}>
      <div className="px-6 py-3 border-b border-slate-100 flex flex-wrap gap-2">
        <StatPill label="PO Lines" value={lines.length} />
...
function SessionDetailModal({ session, onClose }: { session: RecentSession; onClose: () => void }) {
  ...
  useEffect(() => {
    getAdminSessionDetail(session.session_ref)
      .then((data) => {
        setLines(data);
        setExpanded(new Set(data.map((l) => l.supplier_name)));
      })
      .finally(() => setLoading(false));
  }, [session.session_ref]);
  const groups = useMemo(() => {
...
export default function TeamAnalyticsPage() {
  ...
  // Document title
  useEffect(() => {
    document.title = 'All Analytics — PO Expediting | SC Agents';
  }, []);
  ...
  const rateColor =
    !data || data.overallResponseRate === null
      ? 'text-slate-800'
      : data.overallResponseRate >= 70
        ? 'text-[#307c4c]'
        : data.overallResponseRate >= 30
          ? 'text-amber-600'
          : 'text-red-600';
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-8 pb-16 scroll-smooth">
          <div className="max-w-[1400px] mx-auto py-6">
            ...
            <div className="flex items-start justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-0.5">
                  PO Expediting performance across all buyers.
                </p>
              ...
              <div className="flex flex-wrap gap-3 items-end">
                ...
                {filterOpts && (
                  <>
                    <MultiSelect
                      label="Expeditor"
                      options={filterOpts.buyers}
                      selected={buyerEmails}
                      onChange={setBuyerEmails}
                      searchable
                    />
                    <MultiSelect
                      label="Country"
                      options={filterOpts.countries.map((c) => ({ value: c, label: c }))}
                      selected={countries}
                      onChange={setCountries}
                    />
                    <MultiSelect
                      label="P Group"
                      options={filterOpts.segments.map((s) => ({ value: s, label: s }))}
                      selected={segments}
                    ...
                    <MultiSelect
                      label="Supplier"
                      options={filterOpts.suppliers.map((s) => ({ value: s, label: s }))}
                      selected={supplierNames}
            ...
            {!loading && !hasData && (
              <div className="flex flex-col items-center justify-center py-24">
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-12 max-w-md w-full text-center">
                  ...
                  <button
                    onClick={clearFilters}
                    className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-semibold text-white bg-[#307c4c] rounded-lg hover:bg-[#26663e] transition-colors"
                  >
                    Clear Filters
                  </button>
                </div>
            ...
            {!loading && data && hasData && (
              <div
                className={`space-y-8 animate-in fade-in slide-in-from-top-2 duration-300 transition-opacity ${isRefreshing ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}
              >
                {/* KPI cards */}
                <div>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
                    <KpiCard
                      ...
                      value={data.totalLinesExpedited.toLocaleString()}
                      accent
                    />
                    <KpiCard
                      label="Total Suppliers Contacted"
                      value={data.totalSuppliersContacted.toLocaleString()}
                      accent
                    />
                    <KpiCard
                      label="Total Active Buyers"
                      value={data.totalActiveBuyers.toLocaleString()}
                    />
                    <KpiCard
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col gap-1 transition-shadow duration-300 hover:shadow-md">
                      ...
                      <p className={`text-3xl font-bold tracking-tight ${rateColor}`}>
                        {data.overallResponseRate !== null ? `${data.overallResponseRate}%` : '—'}
                      </p>
                    </div>
                    <KpiCard
                      label="Total Expediting Sessions"
