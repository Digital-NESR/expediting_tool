    ...
    <div className="mb-5">
        ...
        {shown.map((opt) => {
          ...
          return (
            <button
              ...
              className={`group flex w-full items-center gap-2.5 rounded-lg px-1.5 py-1.5 text-[13px] transition-colors ${on ? 'bg-[#307c4c]/[0.06]' : 'hover:bg-slate-50'}`}
            >
              <span
                className={`flex h-4 w-4 shrink-0 items-center justify-center rounded-[5px] border transition-all ${on ? 'border-[#307c4c] bg-[#307c4c] text-white' : 'border-slate-300 bg-white group-hover:border-[#6aaf8e]'}`}
              >
                {on && <Icon name="check" className="h-3 w-3" strokeWidth={3} />}
              </span>
              <span
                className={`flex-1 truncate text-left transition-colors ${on ? 'font-semibold text-slate-900' : 'text-slate-500 group-hover:text-slate-700'}`}
              >
                {opt.label}
              </span>
              <span
                className={`rounded-full px-1.5 text-[11px] tabular-nums ${on ? 'bg-[#307c4c]/10 font-semibold text-[#1d4f31]' : 'text-slate-400'}`}
              >
                {counts[opt.value] ?? 0}
              </span>
            </button>
      ...
      {max && options.length > max && (
        <button
          ...
          className="mt-1.5 px-1.5 text-[12px] font-semibold text-[#1d4f31] hover:underline"
        >
...
export default function CatalogListClient({
  ...
  const rows = useMemo(() => {
    let r = entries.filter((e) => {
      ...
      if (q.trim()) {
        const hay =
          `${e.supplier_name} ${e.supplier_code} ${e.description ?? ''} ${e.category_name ?? ''} ${e.subcategory_name ?? ''} ${e.commodity ?? ''} ${e.unspsc_code ?? ''} ${e.code} ${e.uom_name ?? ''} ${e.item_name}`.toLowerCase();
  ...
  const activeFilterCount =
    status.length + cats.length + stypes.length + ctys.length + (expiringOnly ? 1 : 0);
  function clearAll() {
    setStatus([]);
    setCats([]);
    setStypes([]);
  ...
  const selectedSubmittable = selectedRows.filter(
    (r) => r.status === 'Draft' || r.status === 'Rejected',
  );
  const selectedDeactivatable = selectedRows.filter(
  ...
  function exportCsv(which: CatalogEntry[] = rows) {
    ...
    const csv = [
      head.join(','),
      ...which.map((r) =>
        cols.map((c) => `"${csvSafe(r[c] ?? '').replace(/"/g, '""')}"`).join(','),
      ),
    ].join('\n');
    ...
    <CatalogManagerShell
      ...
      headerAction={
        <div className="flex items-center gap-2">
          <button
            onClick={() => exportCsv()}
            className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-600 shadow-sm transition-all hover:border-[#6aaf8e] hover:text-slate-900 active:scale-[0.98]"
          >
          ...
          {canCreate && (
            <Link
              href="/catalog-manager/catalog/add"
              className="inline-flex items-center gap-2 rounded-lg bg-[#307c4c] px-3.5 py-2 text-sm font-semibold text-white shadow-sm shadow-[#307c4c]/25 transition-all hover:bg-[#2b6f44] active:scale-[0.98]"
            >
        ...
        <aside className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm lg:h-full lg:overflow-y-auto">
          <div className="mb-4 flex items-center justify-between">
            <span className="inline-flex items-center gap-2 text-sm font-bold text-slate-900">
              <Icon name="filter" className="h-4 w-4 text-[#307c4c]" /> Filters
            </span>
            {activeFilterCount > 0 && (
              <button
                onClick={clearAll}
                className="text-[12px] font-semibold text-[#1d4f31] hover:underline"
              >
          ...
          >
            <span
              className={`flex h-4 w-4 items-center justify-center rounded border ${expiringOnly ? 'border-amber-500 bg-amber-500 text-white' : 'border-slate-300 bg-white'}`}
            >
          ...
          <FacetGroup
            ...
            options={statusOpts}
            selected={status}
            onToggle={toggle(status, setStatus)}
            counts={counts.st}
          />
          <FacetGroup
            title="Spend type"
            options={typeOpts}
            selected={stypes}
            onToggle={toggle(stypes, setStypes)}
            counts={counts.sty}
          />
          <FacetGroup
        ...
        <div className="flex min-h-0 min-w-0 flex-col rounded-2xl border border-slate-200/80 bg-white shadow-sm lg:h-full">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-[13px] outline-none focus:border-[#307c4c]"
                >
              ...
              <label className="flex items-center gap-2 text-[12px] text-slate-500">
                <input
                  ...
                  className="h-4 w-4 rounded border-slate-300 text-[#307c4c]"
                />
                Include inactive
              </label>
            </div>
          ...
          {selected.size > 0 && (
            <div className="cm-fade-in flex flex-wrap items-center gap-2 border-b border-[#307c4c]/20 bg-gradient-to-r from-[#eaf4ef] to-[#f7fbf9] px-4 py-2.5">
              <span className="text-[13px] font-semibold text-[#1d4f31]">
              ...
              <div className="ml-auto flex flex-wrap items-center gap-2">
                <button
                  ...
                  disabled={bulkBusy}
                  className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-[12.5px] font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-50"
                >
                ...
                {canCreate && selectedSubmittable.length > 0 && (
                  <button
                    onClick={() =>
                      runBulk(() => bulkSubmitEntries(selectedSubmittable.map((r) => r.id)))
                    }
                    disabled={bulkBusy}
                    className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-[12.5px] font-semibold text-[#1d4f31] hover:bg-[#307c4c]/5 disabled:opacity-50"
                  >
                    <Icon name="arrowRight" className="h-3.5 w-3.5" /> Submit (
                    {selectedSubmittable.length})
                  </button>
                )}
                {canCreate && selectedDeactivatable.length > 0 && (
                  <button
                    onClick={() => {
                      if (
                        confirm(
                          `Deactivate ${selectedDeactivatable.length} entr${selectedDeactivatable.length === 1 ? 'y' : 'ies'}?`,
                        )
                      )
                        runBulk(() =>
                          bulkDeactivateEntries(selectedDeactivatable.map((r) => r.id)),
                        );
                    }}
                    disabled={bulkBusy}
                    className="inline-flex items-center gap-1.5 rounded-lg border border-red-200 bg-white px-3 py-1.5 text-[12.5px] font-semibold text-red-600 hover:bg-red-50 disabled:opacity-50"
                  >
                    <Icon name="x" className="h-3.5 w-3.5" /> Deactivate (
                    {selectedDeactivatable.length})
                  </button>
                )}
              </div>
            </div>
          ...
          {homeCountry && rows.some((r) => r.country_code !== homeCountry) && (
            <div className="flex items-start gap-2.5 border-b border-amber-100 bg-amber-50 px-4 py-2.5 text-[12.5px] text-amber-800">
              ...
              <span>
                Some rates below are outside your home country (
                <span className="font-semibold">{homeCountry}</span>) — these are flagged with an
                <span className="mx-1 inline-flex items-center gap-1 rounded bg-amber-100 px-1.5 py-0.5 text-[11px] font-semibold text-amber-700">
            ...
            <EmptyState
              ...
              action={
                activeFilterCount > 0 || q ? (
                  <button
                    onClick={clearAll}
                    className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3.5 py-2 text-sm font-semibold text-slate-700 shadow-sm transition-all hover:border-[#6aaf8e] active:scale-[0.98]"
                  >
                ...
                <tbody>
                  {rows.map((e) => {
                    ...
                    return (
                      <tr
                        key={e.id}
                        onClick={() => router.push(`/catalog-manager/catalog/${e.id}`)}
                        className={`cursor-pointer border-b border-slate-100 transition-colors hover:bg-[#307c4c]/5 ${selected.has(e.id) ? 'bg-[#307c4c]/5' : ''}`}
                      >
                        ...
                        <td className="px-4 py-3">
                          <span className="inline-flex items-center gap-1.5">
                            ...
                            {e.sirion_contract_id && (
                              <Icon name="link" className="h-3.5 w-3.5 text-[#6aaf8e]" />
                            )}
                            {homeCountry && e.country_code !== homeCountry && (
                              <span
                                title={`Outside your home country (${homeCountry})`}
                        ...
                        <td className="whitespace-nowrap px-4 py-3 text-right">
                          <span className="font-mono font-semibold text-slate-900">
                            {fmtMoney(e.unit_price, e.currency_code)}
                          </span>
                        ...
                        <td className="px-4 py-3">
                          <StatusPill status={e.status as CatalogStatus} sm />
                        </td>
                        <td className="whitespace-nowrap px-4 py-3">
                          {e.expiry_date ? (
                            <span
                              className={`text-[12.5px] ${exp ? 'font-semibold text-amber-700' : 'text-slate-500'}`}
                            >
                              {exp && (
                                <Icon name="clock" className="mr-1 inline h-3 w-3 align-[-1px]" />
                              )}
