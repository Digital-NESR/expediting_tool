...
export function buttonClasses(
  ...
  const sizing =
    size === 'sm'
      ? 'gap-1.5 rounded-lg px-3 py-1.5 text-[13px]'
      : 'gap-2 rounded-lg px-3.5 py-2 text-sm';
...
export function StatusPill({ status, sm }: { status: CatalogStatus; sm?: boolean }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full font-semibold ring-1 ring-inset ring-black/[0.04] ${b.pill} ${sm ? 'px-2 py-0.5 text-[11px]' : 'px-2.5 py-1 text-xs'}`}
    >
  ...
  const body = (
    <>
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <span
          className={`inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl transition-transform duration-200 group-hover:scale-110 ${t.icon}`}
        >
      ...
      <div className="mt-2 flex items-center gap-2">
        {delta && (
          <span
            className={`inline-flex items-center gap-0.5 text-[11px] font-semibold ${delta.dir === 'up' ? 'text-[#307c4c]' : delta.dir === 'down' ? 'text-red-600' : 'text-slate-400'}`}
          >
            {delta.dir !== 'flat' && (
              <Icon
                name="trend"
                className={`h-3 w-3 ${delta.dir === 'down' ? 'rotate-180 -scale-x-100' : ''}`}
              />
            )}
            {delta.text}
          </span>
...
export const tableClasses = {
  ...
  trClickable:
    'cursor-pointer transition-colors hover:bg-[#307c4c]/[0.035] focus-visible:bg-[#307c4c]/[0.05] focus-visible:outline-none',
