    ...
    return (
      ...
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          d="M17.8 19.2L16 11l3.5-3.5C21 6 21.5 4 21 3c-1-.5-3 0-4.5 1.5L13 8 4.8 6.2c-.5-.1-.9.1-1.1.5l-.3.5c-.2.5-.1 1 .3 1.3L9 12l-2 3H4l-1 1 3 2 2 3 1-1v-3l3-2 3.5 5.3c.3.4.8.5 1.3.3l.5-.2c.4-.2.6-.6.5-1.1z"
        />
      </svg>
    );
  if (m.includes('sea'))
    return (
      ...
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          d="M2 21c.6.5 1.2 1 2.5 1 2.5 0 2.5-2 5-2s2.5 2 5 2 2.5-2 5-2c1.3 0 1.9.5 2.5 1M19 13V7a2 2 0 00-2-2H7a2 2 0 00-2 2v6M12 10v4M12 2v3"
        />
      </svg>
    );
  if (m.includes('land') || m.includes('lnad'))
    return (
      <svg
        className="w-3.5 h-3.5"
        fill="none"
...
export default function ShipmentsClient({
  ...
  shipments: TiteListShipment[] | null;
  viewOnly?: boolean;
}) {
  ...
  const applyFilters = useCallback(
    (
      ...
      src.filter((s) => {
        ...
        if (movementTypes.length && !movementTypes.includes(s.movement_type ?? '(Blank)'))
          return false;
        ...
        <div className="flex items-end justify-between gap-4 mb-5">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              {rows.length} of {list.length} records · Customs deposit on view:{' '}
              <strong className="tabular-nums">{usdFmt(totalDep)}</strong>
            </p>
          </div>
          {!viewOnly && (
            <button
          ...
          {list.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-sm text-slate-500 mb-5">
                Add your first TI-TE shipment to get started.
              </p>
              {!viewOnly && (
                <button
                  onClick={() => router.push('/ti-te/shipments/new')}
                ...
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50">
                    {[
                      ...
                      <th
                        key={i}
                        className="text-left text-[11px] font-semibold uppercase tracking-wider text-slate-400 px-3 py-2.5 whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
                  ...
                  {rows.map((s) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5">
                        <div className="font-semibold text-slate-900 max-w-[260px] truncate">
                          {s.description || '—'}
                        </div>
                        {s.segment && (
                          <div className="text-[11.5px] text-slate-400">{s.segment}</div>
                        )}
                      </td>
                      ...
                      <td className="px-3 py-2.5 text-[12.5px] whitespace-nowrap">
                        {fmtDate(s.import_date)}
                      </td>
                      <td className="px-3 py-2.5 whitespace-nowrap">
                        <div className="text-[12.5px]">
                          {fmtDate(s.extended_date || s.expiry_date)}
                        </div>
                        {s.extended_date && (
