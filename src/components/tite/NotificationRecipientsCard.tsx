...
function RoleBadge({ role }: { role: string | null }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold border whitespace-nowrap ${cls}`}
    >
      {role || 'Recipient'}
    </span>
  );
...
const INP =
  'w-full border border-slate-200 rounded-lg px-2.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#006B0C]/20 focus:border-[#006B0C] bg-white placeholder:text-slate-400';
...
export default function NotificationRecipientsCard({
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const result = await saveNotificationContacts({
      ...
      <div className="p-4 flex flex-col gap-4">
        ...
        <div>
          ...
          {additionalEdits.map((c, i) => (
            <div
              key={i}
              className="rounded-lg border border-slate-200 p-3 mb-2 flex flex-col gap-2"
            >
          ...
          <EmployeeSearchInput
            ...
            onSelect={(emp: Employee) => {
              if (additionalEdits.some((c) => c.email.toLowerCase() === emp.mail.toLowerCase()))
                return;
              setAdditionalEdits((prev) => [
                ...prev,
                {
