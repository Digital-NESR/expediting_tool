  ...
  useEffect(() => {
    ...
    if (reduceMotion) {
      emit(
        width * 0.5,
        launchY,
        Math.max(20, Math.round(28 * areaScale)),
        -Math.PI / 2,
        1.1,
        diagonalPower * 0.52,
      );
...
export default function DecisionDialog({
  ...
  async function confirm() {
    if (!comment.trim()) {
      setErr('A comment is required to record this decision.');
      return;
    }
    setBusy(true);
    try {
      if (bulk) {
        await bulkDecideEntries(
          bulk.entries.map((e) => e.id),
          comment.trim(),
        );
      } else {
    ...
    <div className="fixed inset-0 z-[90] flex items-start justify-center p-4 pt-[8vh]">
      ...
      <div className="cm-scale-in relative z-10 w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-950/20">
        <div className="flex items-center gap-3 border-b border-slate-100 px-5 pb-4 pt-5">
          <span
            className={`inline-flex h-10 w-10 items-center justify-center rounded-xl ring-4 ${approving ? 'bg-[#307c4c]/10 text-[#307c4c] ring-[#307c4c]/5' : 'bg-red-50 text-red-600 ring-red-500/5'}`}
          >
          ...
          <div className="min-w-0">
            <h2 className="text-base font-bold text-slate-900">
              {isBulk
                ? `Approve ${bulk!.entries.length} entries`
                : approving
                  ? 'Approve catalog entry'
                  : 'Reject or request revision'}
            </h2>
            <p className="truncate text-[12.5px] text-slate-500">
              {isBulk ? bulk!.supplier : `${entry.code} · ${entry.supplier_name}`}
            </p>
          </div>
        </div>
        ...
        <div className="px-5 py-4">
          {isBulk ? (
            <div className="mb-3 max-h-48 overflow-y-auto rounded-lg border border-slate-200">
              {bulk!.entries.map((e, i) => (
                <div
                  key={e.id}
                  className={`flex items-center gap-2 px-3 py-2 text-[12.5px] ${i ? 'border-t border-slate-100' : ''}`}
                >
            ...
            <div className="mb-3 flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 p-3">
              <div className="min-w-0">
                ...
                <div className="text-[12px] text-slate-400">
                  {entry.category_name} · {entry.country_flag} {entry.country_name}
                </div>
              </div>
              <div className="ml-3 shrink-0 text-right">
                ...
                <div className="text-[11px] text-slate-400">
                  {entry.currency_code} / {entry.uom_name} · ≈${fmtUsd(entry.usd_equivalent)}
                </div>
              </div>
            </div>
          )}
          ...
          {!approving && !isBulk && (
            <div className="mb-4 grid grid-cols-2 gap-2 rounded-lg border border-slate-200 bg-slate-50 p-1">
              {(
                [
                  ['reject', 'Reject', 'x'],
                  ['revise', 'Request revision', 'revise'],
                ] as const
              ).map(([v, label, ic]) => (
                <button
                  key={v}
                  onClick={() => setMode(v)}
                  className={`inline-flex items-center justify-center gap-1.5 rounded-md px-3 py-2 text-[13px] font-semibold ${mode === v ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400'}`}
                >
          ...
          <label className="flex flex-col gap-1.5">
            <span className="text-[12.5px] font-semibold text-slate-600">
              {approving ? 'Approval comment' : 'Reason (shared with submitter)'}
            ...
            <textarea
              ...
              placeholder={
                isBulk
                  ? 'e.g. Rate card reviewed against quotation — all lines within benchmark.'
                  : approving
                    ? 'e.g. Within benchmark for the region — approved.'
        ...
        <div className="flex justify-end gap-2.5 border-t border-slate-100 bg-slate-50/60 px-5 py-4">
          ...
          >
            {busy ? (
              ...
              <Icon
                name={approving ? 'check' : mode === 'revise' ? 'revise' : 'x'}
                className="h-4 w-4"
              />
            )}
            {isBulk
              ? `Approve all ${bulk!.entries.length}`
              : approving
                ? 'Approve & activate'
                : mode === 'revise'
