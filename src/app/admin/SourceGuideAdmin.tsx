...
import {
  getSourceGuideAccessRequests,
  getSourceGuidePendingCount,
  approveSourceGuideAccessRequest,
  rejectSourceGuideAccessRequest,
  revokeSourceGuideAccess,
  deleteSourceGuideAccessRequest,
  getChampionsByCountry,
  addChampion,
  updateChampion,
  removeChampion,
  getGuides,
  getSourceGuideAnalytics,
  getCoverageGapsSummary,
  getSourceGuideInsights,
  getSourceGuideAuditLog,
  getUserActivity,
...
export function SourceGuideAccessApprovalsClient({
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    const [reqs, cnt] = await Promise.all([
      getSourceGuideAccessRequests(),
      getSourceGuidePendingCount(),
    ]);
    setRequests(reqs);
  ...
  return (
    <div>
      <h2 className="mb-1 text-lg font-bold tracking-tight text-slate-900">
        SourceGuide · Access Approvals
      </h2>
      <p className="mb-6 text-[13px] text-slate-500">
        Grant general users access to view all country guides (read-only). Champions are managed
        separately under the Champions tab.
      </p>
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {r.status !== 'Approved' && (
                    <button
                      disabled={busy}
                      onClick={() => run(() => approveSourceGuideAccessRequest(r.user_email))}
                  ...
                  {r.status === 'Pending' && (
                    <button
                      disabled={busy}
                      onClick={() => run(() => rejectSourceGuideAccessRequest(r.user_email))}
                  ...
                  {r.status === 'Approved' && (
                    <button
                      disabled={busy}
                      onClick={() => run(() => revokeSourceGuideAccess(r.user_email))}
                      className="rounded-lg border border-red-200 px-3 py-1.5 text-[12.5px] font-semibold text-red-600 hover:bg-red-50"
                    >
                      Revoke
                    </button>
                  )}
                  <button
                    ...
                    onClick={() => {
                      if (confirm('Delete this request?'))
                        run(() => deleteSourceGuideAccessRequest(r.user_email));
                    }}
                    className="rounded-lg border border-slate-200 px-3 py-1.5 text-[12.5px] font-semibold text-slate-400 hover:bg-slate-50"
                  >
                    Delete
                  </button>
                </div>
...
export function SourceGuideChampionsClient() {
  ...
  const [editDraft, setEditDraft] = useState<{ name: string; email: string }>({
    name: '',
    email: '',
  });
  ...
  if (loading)
    return <div className="py-16 text-center text-sm text-slate-400">Loading champions…</div>;
  ...
  return (
    <div>
      ...
      <p className="mb-6 text-[13px] text-slate-500">
        Assign champions per country. A champion gets access automatically on login (matched by
        email) and can edit mappings only for their country.
      </p>
          ...
          >
            <div className="flex items-center gap-2.5 border-b border-slate-100 bg-slate-50 px-5 py-3">
              ...
              <span className="ml-auto font-mono text-[11.5px] text-slate-400">
                {c.champions.length} champion{c.champions.length === 1 ? '' : 's'}
              </span>
            </div>
            <div className="divide-y divide-slate-50">
              {c.champions.map((ch) =>
                editingId === ch.id ? (
                  <div
                    key={ch.id}
                    className="flex flex-wrap items-center gap-2 bg-[#eaf4ef]/40 px-5 py-2.5"
                  >
                    <input
                      value={editDraft.name}
                      onChange={(e) => setEditDraft((d) => ({ ...d, name: e.target.value }))}
                      placeholder="Name"
                      className="w-40 rounded-lg border border-slate-200 px-3 py-1.5 text-[13px] outline-none focus:border-[#6AAF8E]"
                    />
                    <input
                      value={editDraft.email}
                      onChange={(e) => setEditDraft((d) => ({ ...d, email: e.target.value }))}
                      placeholder="email@nesr.com"
                      className="w-56 rounded-lg border border-slate-200 px-3 py-1.5 text-[13px] outline-none focus:border-[#6AAF8E]"
                    />
                    <button
                      disabled={busy}
                    ...
                    <button
                      ...
                      className="rounded-lg border border-slate-200 px-3 py-1.5 text-[12.5px] font-semibold text-slate-600 hover:bg-white"
                    >
                      Cancel
                    </button>
                  </div>
                  ...
                  <div key={ch.id} className="flex items-center gap-3 px-5 py-2.5">
                      ...
                      <span className="ml-2 text-[12px] text-slate-500">
                        {ch.email || (
                          <span className="italic text-amber-600">
                            no email, add one to grant access
                          </span>
                        )}
                      </span>
                    </div>
                    ...
                    <button
                      disabled={busy}
                      onClick={() => {
                        if (confirm(`Remove ${ch.name}?`)) run(() => removeChampion(ch.id));
                      }}
                      className="rounded-lg border border-slate-200 px-2.5 py-1 text-[12px] font-semibold text-slate-400 hover:bg-slate-50 hover:text-red-600"
                    >
                      Remove
                    </button>
                  </div>
              ...
              {c.champions.length === 0 && (
                <div className="px-5 py-2.5 text-[12.5px] text-slate-400">No champions yet.</div>
              )}
            </div>
            <div className="flex flex-wrap items-center gap-2 border-t border-slate-100 bg-slate-50/60 px-5 py-3">
              <input
                value={draft[c.country]?.name ?? ''}
                onChange={(e) => setField(c.country, 'name', e.target.value)}
                placeholder="Name"
                className="w-40 rounded-lg border border-slate-200 px-3 py-1.5 text-[13px] outline-none focus:border-[#6AAF8E]"
              />
              <input
                value={draft[c.country]?.email ?? ''}
                onChange={(e) => setField(c.country, 'email', e.target.value)}
                placeholder="email@nesr.com"
                className="w-56 rounded-lg border border-slate-200 px-3 py-1.5 text-[13px] outline-none focus:border-[#6AAF8E]"
              />
              <button
                disabled={busy}
                onClick={() => add(c.country)}
                className="rounded-lg px-3 py-1.5 text-[12.5px] font-semibold text-white disabled:opacity-50"
                style={{ background: BRAND }}
              >
                Add champion
...
export function SourceGuideGuidesClient() {
  ...
  if (loading)
    return <div className="py-16 text-center text-sm text-slate-400">Loading source guides…</div>;

  return (
    <div>
      <h2 className="mb-1 text-lg font-bold tracking-tight text-slate-900">
        SourceGuide · Country Guides
      </h2>
      <p className="mb-6 text-[13px] text-slate-500">
        Status and coverage of every published country source guide.
      </p>

      {stale > 0 && (
        <div
          className="mb-4 rounded-xl border px-4 py-3 text-[13px]"
          style={{ borderColor: '#f6c453', background: '#f6efdf', color: '#7a5616' }}
        >
          <b>
            {stale} guide{stale === 1 ? '' : 's'} exceed the 90-day staleness threshold.
          </b>{' '}
          Owners should review and re-publish.
        </div>
      )}
        ...
        {guides.map((g) => {
          ...
          const ss =
            g.status === 'Published'
              ...
              : g.status === 'Draft'
                ? { bg: '#f6efdf', col: '#b07d24' }
                : { bg: '#ececed', col: '#58595b' };
          return (
            <div
              key={g.country}
              className="grid grid-cols-[1.4fr_0.8fr_1fr_1.1fr_0.8fr] items-center gap-4 border-b border-slate-50 px-5 py-3.5 last:border-b-0"
            >
              <div className="flex items-center gap-3">
                <span
                  className="h-[17px] w-[24px] shrink-0 rounded-sm"
                  style={{ background: g.tone ?? '#999' }}
                />
                <div>
                  <div className="text-[14px] font-semibold text-slate-900">{g.name}</div>
                  <div className="font-mono text-[11.5px] text-slate-400">
                    {g.version} · {g.mappings.toLocaleString()} mappings
                  </div>
                </div>
              </div>
              <div>
              ...
              <div className="text-[13px]" style={{ color: isStale ? '#b07d24' : '#58595b' }}>
...
export function SourceGuideAnalyticsClient() {
  ...
  if (!data || !insights)
    return <div className="py-16 text-center text-sm text-slate-400">Loading analytics…</div>;
      ...
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-[13px] text-slate-500">
            Coverage, supplier reach and a full record of every change.
          </p>
        </div>
        <span
          className="rounded-full bg-[#eaf4ef] px-3 py-1.5 text-[12px] font-semibold"
          style={{ color: BRAND }}
        >
          {insights.activity30d.toLocaleString()} change{insights.activity30d === 1 ? '' : 's'} in
          the last 30 days
        </span>
      </div>
...
function BarRow({
  ...
  return (
    <div className="flex items-center gap-3">
      <span
        className={`${labelWidth} shrink-0 truncate text-[12.5px] text-slate-600`}
        title={label}
      >
        {label}
      </span>
      ...
      <span className="w-14 shrink-0 text-right font-mono text-[11.5px] text-slate-500">
...
function OverviewTab({ data, insights }: { data: SgAnalytics; insights: SgInsights }) {
  ...
  const coverage = pct(
    insights.coverageOverall.coveredAnywhere,
    insights.coverageOverall.catalogue,
  );
  const prefShare = pct(insights.tier.preferred, totalTier);
  const noChampion = Math.max(
    0,
    insights.champions.countriesTotal - insights.champions.withChampion,
  );
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
        ...
        <Kpi
          ...
          value={data.stats.suppliers.toLocaleString()}
          sub={`${insights.avl.total.toLocaleString()} in AVL`}
        />
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <Kpi
          ...
          value={`${coverage}%`}
          sub={`${insights.coverageOverall.coveredAnywhere.toLocaleString()} of ${insights.coverageOverall.catalogue.toLocaleString()} mapped`}
          tone={coverage >= 60 ? 'good' : coverage >= 30 ? 'warn' : 'bad'}
        />
        <Kpi
          label="Preferred share"
          value={`${prefShare}%`}
          sub={`${insights.tier.preferred.toLocaleString()} preferred · ${insights.tier.backup.toLocaleString()} backup`}
          tone="good"
        />
        <Kpi
          label="Single-source pairs"
          value={insights.singleSourcePairs.toLocaleString()}
          sub="country + commodity with 1 supplier"
          tone={insights.singleSourcePairs > 0 ? 'warn' : 'good'}
        />
        <Kpi
          label="Countries missing champion"
          value={noChampion.toLocaleString()}
          sub={`${insights.champions.withChampion} of ${insights.champions.countriesTotal} have a champion`}
          tone={noChampion > 0 ? 'warn' : 'good'}
        />
      </div>
        ...
        <div className="space-y-5">
          ...
          <Panel title="Tier mix">
            <div className="flex overflow-hidden rounded-full">
              <div
                className="h-4"
                style={{ width: `${prefShare}%`, background: BRAND }}
                title={`Preferred ${insights.tier.preferred}`}
              />
              <div
                className="h-4 flex-1"
                style={{ background: '#9CC7B0' }}
                title={`Backup ${insights.tier.backup}`}
              />
            </div>
            <div className="mt-3 flex items-center justify-between text-[12.5px] text-slate-600">
              <span className="flex items-center gap-1.5">
                ...
                Preferred {insights.tier.preferred.toLocaleString()}
              </span>
              <span className="flex items-center gap-1.5">
                <span className="h-2.5 w-2.5 rounded-full" style={{ background: '#9CC7B0' }} />{' '}
                Backup {insights.tier.backup.toLocaleString()}
              </span>
            </div>
          </Panel>
        </div>
...
function CoverageTab({ gaps, insights }: { gaps: SgCoverageGap[]; insights: SgInsights }) {
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <Kpi
          label="Catalogue commodities"
          value={insights.coverageOverall.catalogue.toLocaleString()}
        />
        <Kpi
          label="Covered somewhere"
          value={insights.coverageOverall.coveredAnywhere.toLocaleString()}
          sub={`${coverage}% of catalogue`}
          tone={coverage >= 60 ? 'good' : coverage >= 30 ? 'warn' : 'bad'}
        />
        <Kpi
          label="No-preferred pairs"
          value={insights.noPreferredPairs.toLocaleString()}
          sub="covered but only backups"
          tone={insights.noPreferredPairs > 0 ? 'warn' : 'good'}
        />
        <Kpi
          label="No-backup pairs"
          value={noBackupTotal.toLocaleString()}
          sub="a preferred, but no fallback"
          tone={noBackupTotal > 0 ? 'warn' : 'good'}
        />
      </div>

      {/* Coverage gaps by country */}
      <Panel
        title="Coverage gaps by country"
        subtitle={`Of the ${(gaps[0]?.catalogueTotal ?? insights.coverageOverall.catalogue).toLocaleString()} commodities in the full taxonomy, how many each country covers. Click a number to jump into the mappings workspace.`}
      >
        <div className="overflow-hidden rounded-lg border border-slate-200">
          <div className="grid grid-cols-[1.4fr_1.4fr_0.7fr_0.9fr_0.85fr_0.85fr] gap-3 border-b border-slate-100 bg-slate-50 px-4 py-2 font-mono text-[10px] font-semibold uppercase tracking-[0.1em] text-slate-400">
            ...
            <span className="text-right">No preferred</span>
            <span className="text-right">No backup</span>
            <span className="text-right">Missing</span>
          </div>
          {gaps.map((g) => (
            <div
              key={g.country}
              className="grid grid-cols-[1.4fr_1.4fr_0.7fr_0.9fr_0.85fr_0.85fr] items-center gap-3 border-b border-slate-50 px-4 py-2.5 last:border-b-0"
            >
              ...
              <span className="flex items-center gap-2">
                <span className="h-2 flex-1 overflow-hidden rounded-full bg-slate-100">
                  <span
                    ...
                    style={{ width: `${Math.round(g.coverage * 100)}%`, background: BRAND }}
                  />
                </span>
              ...
              <a
                href={`/sourceguide/mappings?country=${g.country}&gap=no-preferred`}
                className={`text-right font-mono text-[12px] font-semibold ${g.noPreferred ? 'text-amber-600 hover:underline' : 'text-slate-300'}`}
              >
                {g.noPreferred}
              </a>
              <span
                className={`text-right font-mono text-[12px] font-semibold ${g.noBackup ? 'text-slate-600' : 'text-slate-300'}`}
              >
                {g.noBackup}
              </span>
              <a
                href={`/sourceguide/mappings?country=${g.country}&gap=missing`}
                className="text-right font-mono text-[12px] font-semibold hover:underline"
                style={{ color: g.missing ? '#64748b' : '#cbd5e1' }}
              >
                {g.missing}
              </a>
            </div>
      ...
      <Panel
        ...
        subtitle="Share of each category's commodities that are mapped in at least one country."
      >
          ...
          {cats.map((c) => {
            ...
            return (
              <div key={c.category} className="flex items-center gap-3">
                <span
                  className="w-40 shrink-0 truncate text-[12.5px] text-slate-600"
                  title={c.category}
                >
                  {c.category}
                </span>
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <Kpi
          label="Suppliers mapped"
          value={insights.avl.mapped.toLocaleString()}
          sub={`${utilisation}% of the AVL is in use`}
          tone="good"
        />
        <Kpi
          label="Multi-country suppliers"
          value={insights.multiCountrySuppliers.toLocaleString()}
          sub="active in more than one country"
        />
        <Kpi
          label="Single-source pairs"
          value={insights.singleSourcePairs.toLocaleString()}
      ...
      <Panel
        title="AVL utilisation"
        subtitle="How much of the Approved Vendor List is actually mapped to a commodity somewhere."
      >
        <div className="flex overflow-hidden rounded-full">
        ...
        <div className="mt-2 text-[12.5px] text-slate-500">
          {insights.avl.mapped.toLocaleString()} of {insights.avl.total.toLocaleString()} approved
          vendors mapped ({utilisation}%)
        </div>
      </Panel>
        ...
        <Panel title="Top suppliers by mappings">
          <div className="space-y-2.5">
            ...
            {data.topSuppliers.length === 0 && (
              <div className="text-[12.5px] text-slate-400">No supplier mappings yet.</div>
            )}
          </div>
        </Panel>
            ...
            {insights.topMultiCountry.map((s) => (
              <div key={s.code} className="flex items-center gap-3">
                ...
                <div className="h-3.5 flex-1 overflow-hidden rounded-full bg-slate-100">
                  <div
                    className="h-full rounded-full"
                    style={{ width: `${(s.mappings / maxMulti) * 100}%`, background: BRAND }}
                  />
                </div>
                <span className="w-24 shrink-0 text-right font-mono text-[11px] text-slate-500">
                  {s.countries} ctry · {s.mappings}
                </span>
              </div>
            ))}
            {insights.topMultiCountry.length === 0 && (
              <div className="text-[12.5px] text-slate-400">
                No suppliers span multiple countries yet.
              </div>
            )}
          </div>
        </Panel>
...
function ChangeRow({ e }: { e: SgAuditEntry }) {
  ...
  return (
    <div className="flex items-start gap-2.5 py-1.5">
      <span
        className="mt-[1px] shrink-0 rounded px-1.5 py-0.5 text-[10px] font-semibold"
        style={{ background: st.bg, color: st.col }}
      >
        {e.action}
      </span>
      <span
        className="min-w-0 flex-1 truncate text-[12.5px] text-slate-600"
        title={e.details ?? ''}
      >
        {e.details || 'No detail'}
      </span>
      <span className="shrink-0 text-[11.5px] font-medium text-slate-600">
        {e.performedBy || 'System'}
      </span>
      <span
        className="w-16 shrink-0 text-right text-[11px] text-slate-400"
        title={fullTime(e.performedAt)}
      >
        {timeAgo(e.performedAt)}
      </span>
    </div>
  );
}
...
function AuditTab({ entries, loading }: { entries: SgAuditEntry[] | null; loading: boolean }) {
  ...
  const filtered = useMemo(
    () =>
      list.filter((e) => {
        ...
        if (q.trim()) {
          const hay =
            `${e.action} ${e.details ?? ''} ${e.countryName ?? ''} ${e.commodityName ?? ''} ${e.performedBy ?? ''}`.toLowerCase();
  ...
  const grouped = useMemo(() => {
    const m = new Map<
      string,
      {
        name: string;
        tone: string | null;
        coms: Map<number, { label: string; entries: SgAuditEntry[] }>;
        countryLevel: SgAuditEntry[];
        total: number;
      }
    >();
    for (const e of list) {
      ...
      if (e.commodityId != null) {
        ...
        if (com) com.entries.push(e);
          ...
          g.coms.set(e.commodityId, {
            label: e.commodityName ?? `Commodity #${e.commodityId}`,
            entries: [e],
          });
      } else {
  ...
  if (loading && entries === null)
    return <div className="py-16 text-center text-sm text-slate-400">Loading audit log…</div>;
    ...
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <div className="flex overflow-hidden rounded-lg border border-slate-200">
          {(
            [
              ['country', 'Mapping changes by country'],
      ...
      {list.length === 0 ? (
        <div className="rounded-xl border border-slate-200 bg-white py-16 text-center text-sm text-slate-400">
          No changes recorded yet. Mapping edits, champion changes and access decisions will appear
          here.
        </div>
      ) : view === 'country' ? (
        grouped.length === 0 ? (
        ...
        <>
          ...
          <div className="mb-4 flex flex-wrap items-center gap-2">
            ...
            <input
              ...
              placeholder="Search action, supplier, commodity, country…"
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
              {filtered.map((e, i) => {
                ...
                return (
                  <div
                    key={e.id}
                    className={`flex items-start gap-3 px-4 py-3 ${i > 0 ? 'border-t border-slate-50' : ''}`}
                  >
                    <span
                      className="mt-0.5 w-[68px] shrink-0 rounded-full px-2 py-0.5 text-center text-[10px] font-bold uppercase tracking-wide"
                      style={{ background: st.bg, color: st.col }}
                    >
                      {st.label}
                    </span>
                    ...
                    <div className="shrink-0 text-right">
                      ...
                      <div className="text-[11px] text-slate-400" title={fullTime(e.performedAt)}>
                        {timeAgo(e.performedAt)}
                      </div>
                    </div>
                  </div>
...
function PeopleTab({ rows, loading }: { rows: SgUserActivity[] | null; loading: boolean }) {
  if (loading && rows === null)
    return <div className="py-16 text-center text-sm text-slate-400">Loading user activity…</div>;
  const list = rows ?? [];
  ...
  return (
    <div className="space-y-5">
      ...
      <Panel
        ...
        subtitle="Views and searches show how much each person uses the tool; edits are recorded changes (mappings, champions, access). Anonymous or signed-out traffic is not counted."
      >
        {list.length === 0 ? (
          <div className="py-8 text-center text-[13px] text-slate-400">
            No usage recorded yet. Views and searches start logging as people browse the tool.
          </div>
        ) : (
          <div className="overflow-x-auto">
            <div className="min-w-[720px] overflow-hidden rounded-lg border border-slate-200">
              <div className="grid grid-cols-[1.7fr_1.3fr_0.7fr_0.8fr_0.7fr_0.8fr_1fr] gap-3 border-b border-slate-100 bg-slate-50 px-4 py-2 font-mono text-[10px] font-semibold uppercase tracking-[0.1em] text-slate-400">
                ...
                <span className="text-right" title="Distinct countries this person edited in">
                  Countries edited
                </span>
              ...
              {list.map((r) => (
                <div
                  key={r.user}
                  className="grid grid-cols-[1.7fr_1.3fr_0.7fr_0.8fr_0.7fr_0.8fr_1fr] items-center gap-3 border-b border-slate-50 px-4 py-2.5 last:border-b-0"
                >
                  <span
                  ...
                  <span className="flex items-center gap-2">
                    <span className="h-2 flex-1 overflow-hidden rounded-full bg-slate-100">
                      <span
                        className="block h-full rounded-full"
                        style={{ width: `${(score(r) / maxScore) * 100}%`, background: BRAND }}
                      />
                    </span>
                  ...
                  <span
                    className={`text-right font-mono text-[12px] ${r.views ? 'text-slate-700 font-semibold' : 'text-slate-300'}`}
                  >
                    {r.views}
                  </span>
                  <span
                    className={`text-right font-mono text-[12px] ${r.searches ? 'text-slate-700 font-semibold' : 'text-slate-300'}`}
                  >
                    {r.searches}
                  </span>
                  <span
                    className={`text-right font-mono text-[12px] ${r.edits ? 'text-slate-600' : 'text-slate-300'}`}
                    title={`${r.mappings} mapping, ${r.champions} champion, ${r.access} access`}
                  >
                    {r.edits}
                  </span>
                  <span
                    className={`text-right font-mono text-[12px] ${r.countries ? 'text-slate-600' : 'text-slate-300'}`}
                  >
                    {r.countries}
                  </span>
                  <span
                    className="text-right text-[11.5px] text-slate-500"
                    title={fullTime(r.lastActive)}
                  >
