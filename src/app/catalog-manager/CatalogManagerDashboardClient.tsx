...
function HeroLink({
  ...
  return (
    <Link
      href={href}
      className={`inline-flex items-center justify-center gap-2 rounded-full px-4 py-2 text-sm font-semibold transition-all ${classes}`}
    >
...
export default function CatalogManagerDashboardClient({
  ...
  const countryTiles = (
    scope === 'ALL' ? countries : countries.filter((c) => c.code === scope)
  ).slice(0, 8);
      ...
      <div className="cm-stagger space-y-6">
          ...
          <div className="relative grid w-full flex-1 items-center gap-8 px-5 py-6 sm:px-7 lg:grid-cols-[minmax(0,1fr)_410px] lg:px-8 lg:py-8">
            <div className="min-w-0">
              ...
              <h1 className="max-w-[760px] text-[30px] font-bold leading-[1.08] tracking-tight text-slate-950 sm:text-[38px]">
                ...
                <span className="bg-gradient-to-r from-[#307c4c] to-[#6aaf8e] bg-clip-text text-transparent">
                  in seconds.
                </span>
              </h1>
              <p className="mt-3 max-w-[650px] text-[15px] leading-relaxed text-slate-600">
                Search the services catalog &amp; PIR / inventory, jump into country views, or load
                a spreadsheet-backed catalog migration for {data.scope}.
              </p>

              <div ref={searchBox} className="relative z-20 mt-6 max-w-[680px]">
                <form
                  onSubmit={submitSearch}
                  className="flex items-center gap-3 rounded-2xl bg-white/90 px-4 py-3 shadow-lg shadow-[#307c4c]/10 ring-1 ring-slate-200 backdrop-blur-sm transition-shadow duration-300 focus-within:shadow-xl focus-within:shadow-[#307c4c]/15 focus-within:ring-2 focus-within:ring-[#307c4c]/25"
                >
                  ...
                  <button
                    type="submit"
                    className="rounded-full bg-[#307c4c] px-4 py-2 text-[13px] font-semibold text-white shadow-sm shadow-[#307c4c]/25 transition-all hover:bg-[#2b6f44] active:scale-[0.97]"
                  >
                    Search
                  </button>
                </form>
                    ...
                    {loading && !hasResults ? (
                      ...
                      <>
                        {res.entries.length > 0 && (
                          <div className="px-2">
                            <p className="px-2 py-1.5 text-[10.5px] font-semibold uppercase tracking-wider text-slate-400">
                              Services catalog
                            </p>
                            {res.entries.map((e) => (
                              <button
                                ...
                                className="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-left transition-colors hover:bg-[#307c4c]/5"
                              >
                                ...
                                <span className="min-w-0 flex-1">
                                  ...
                                  <span className="block truncate text-[11px] text-slate-400">
                                    {e.supplier_name} · {e.country_code}
                                  </span>
                                </span>
                          ...
                          <div className="px-2 pt-1">
                            ...
                            {res.pir.map((p, i) => (
                              <button
                                ...
                                type="button"
                                onClick={() => go(pirSearchHref(p))}
                                className="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-left transition-colors hover:bg-[#307c4c]/5"
                              >
                                <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-cyan-50 text-cyan-700">
                                  <Icon name="sheet" className="h-3.5 w-3.5" />
                                </span>
                                <span className="min-w-0 flex-1">
                                  <span className="block truncate text-[13px] font-medium text-slate-800">
                                    {p.material_description || p.product_number || '—'}
                                  </span>
                                  <span className="block truncate text-[11px] text-slate-400">
                                    {p.supplier_name}
                                    {p.country ? ` · ${p.country}` : ''}
                          ...
                          <div className="px-2 pt-1">
                            ...
                            {res.suppliers.map((s) => (
                              <button
                                ...
                                className="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-left transition-colors hover:bg-[#307c4c]/5"
                              >
                                <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[#eaf4ef] text-[#307c4c]">
                        ...
                        <div className="mt-1 flex items-center justify-between gap-2 border-t border-slate-100 px-3 pt-2 text-[12px]">
                          <button
                            type="button"
                            onClick={() => go(catalogHref(scope, { q: query.trim() }))}
                            className="inline-flex items-center gap-1 font-semibold text-[#1d4f31] hover:underline"
                          >
                            All in Services catalog{' '}
                            <Icon name="arrowRight" className="h-3.5 w-3.5" />
                          </button>
                          <button
                            type="button"
                            onClick={() =>
                              go(`/catalog-manager/pir?q=${encodeURIComponent(query.trim())}`)
                            }
                            className="inline-flex items-center gap-1 font-semibold text-cyan-700 hover:underline"
                          >
                            All in PIR / Inventory{' '}
                            <Icon name="arrowRight" className="h-3.5 w-3.5" />
                          </button>
                        </div>
                      </>
              ...
              <div className="mt-6 flex flex-wrap gap-2.5">
                <HeroLink href={catalogHref(scope, { status: 'Active' })} icon="catalog" primary>
                  Browse catalog
                </HeroLink>
                {actor.canCreate && (
                  <HeroLink href="/catalog-manager/catalog/import" icon="upload">
                    Excel new entries
                  </HeroLink>
                )}
                {actor.canAdmin && (
                  <HeroLink href="/catalog-manager/admin" icon="admin">
                    Catalog migration
                  </HeroLink>
                )}
              </div>
            ...
            <div className="rounded-2xl border border-white/80 bg-white/85 p-4 shadow-xl shadow-[#307c4c]/10 backdrop-blur-sm">
              ...
              <div className="mt-5 grid grid-cols-3 gap-2">
                <div className="rounded-xl bg-[#eaf4ef] p-3 transition-transform duration-200 hover:scale-[1.03]">
                  <p className="text-2xl font-bold leading-none text-[#1d4f31] tabular-nums">
                ...
                <div className="rounded-xl bg-slate-100 p-3 transition-transform duration-200 hover:scale-[1.03]">
                  <p className="text-2xl font-bold leading-none text-slate-900 tabular-nums">
                    {data.supplierCount.toLocaleString()}
                  </p>
                ...
                <div className="rounded-xl bg-cyan-50 p-3 transition-transform duration-200 hover:scale-[1.03]">
                  <p className="text-2xl font-bold leading-none text-cyan-800 tabular-nums">
              ...
              <div className="mt-5 space-y-3">
                {topCategories.length === 0 ? (
                  ...
                  topCategories.map((cat) => (
                    <Link
                      key={cat.name}
                      href={catalogHref(scope, { status: 'Active', category: cat.name })}
                      className="block rounded-xl border border-slate-100 px-3 py-2.5 transition-all hover:border-[#6aaf8e] hover:bg-[#f7fbf9]"
                    >
          ...
          <section>
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-[15px] font-bold tracking-tight text-slate-900">
                Jump to country catalog
              </h2>
              <Link
                href={catalogHref('ALL')}
                className="group inline-flex items-center gap-1 text-[12.5px] font-semibold text-[#1d4f31] hover:underline"
              >
                All countries{' '}
                <Icon
                  name="chevRight"
            ...
            <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-8">
              {countryTiles.map((country) => (
                ...
                >
                  <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#eaf4ef] text-[15px] transition-transform duration-200 group-hover:scale-110">
        ...
        <section className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard
            ...
            sub={`Across ${data.categoryCount} categories`}
            icon="catalog"
            href={catalogHref(scope, { status: 'Active' })}
          />
          <StatCard
            label="Suppliers"
            value={data.supplierCount.toLocaleString()}
          ...
          <StatCard
            ...
            sub={data.expiringCount ? 'Renew before they lapse' : 'Nothing lapsing soon'}
            icon="clock"
            tone="amber"
            href={catalogHref(scope, { expiring: '1' })}
          />
          <StatCard
            label="Pending approval"
            value={data.pendingCount.toLocaleString()}
            sub={data.pendingCount ? 'Awaiting sign-off' : 'Queue is clear'}
            icon="approve"
            tone="ink"
            href={
              scope !== 'ALL'
                ? `/catalog-manager/approvals?country=${scope}`
                : '/catalog-manager/approvals'
            }
          />
        </section>
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)_minmax(320px,0.85fr)]">
            ...
            {data.byCategory.length === 0 ? (
              ...
              <div className="space-y-1.5">
                {data.byCategory.slice(0, 7).map((category) => (
                  <Link
                    key={category.name}
                    href={catalogHref(scope, { status: 'Active', category: category.name })}
                    className="grid grid-cols-[minmax(110px,180px)_1fr_38px] items-center gap-3 rounded-lg px-2 py-2 transition-colors hover:bg-[#f7fbf9]"
                  >
                    <span className="truncate text-[12.5px] font-medium text-slate-600">
                      {category.name}
                    </span>
                    <Meter pct={(category.count / maxCat) * 100} className="h-2.5" />
                    <span className="text-right text-[12.5px] font-semibold tabular-nums text-slate-900">
                      {category.count}
                    </span>
                  </Link>
                ))}
          ...
          <Card className="p-5">
            <CardHeader
              ...
              action={
                <span className="rounded-full bg-amber-50 px-2 py-0.5 text-[10.5px] font-semibold text-amber-700 ring-1 ring-inset ring-amber-600/10">
                  &lt;= 30 days
                </span>
              }
            />
              ...
              <div className="divide-y divide-slate-100">
                {data.expiringSoon.slice(0, 5).map((entry) => {
                  ...
                  return (
                    <Link
                      key={entry.id}
                      href={`/catalog-manager/catalog/${entry.id}`}
                      className="flex items-center gap-3 rounded-lg py-2.5 transition-colors hover:bg-slate-50"
                    >
                      <span
                        className={`flex h-10 w-10 shrink-0 flex-col items-center justify-center rounded-xl font-bold leading-none ring-1 ring-inset ${days <= 7 ? 'bg-red-50 text-red-600 ring-red-600/10' : 'bg-amber-50 text-amber-700 ring-amber-600/10'}`}
                      >
          ...
          <Card className="p-5">
            <CardHeader
              ...
              action={
                <Link
                  href="/catalog-manager/audit"
                  className="group inline-flex items-center gap-0.5 text-[11.5px] font-semibold text-[#1d4f31] hover:underline"
                >
                  Log{' '}
                  <Icon
                    name="chevRight"
            ...
            <div className="divide-y divide-slate-100">
              ...
              {data.recent.length === 0 && (
                <EmptyState
                  ...
                  sub="Changes to the catalog will appear here."
                />
              )}
            </div>
          </Card>
