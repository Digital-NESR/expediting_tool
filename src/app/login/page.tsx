      ...
      <div className="relative z-10 w-full px-4 sm:max-w-md sm:px-0">
        <div className="bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 md:px-8 md:py-10 flex flex-col items-center gap-7">
          ...
          <div className="flex flex-col items-center gap-5">
            ...
            >
              <Image src={images.logo} alt={text.appName} fill className="object-cover" priority />
            </div>
            <div className="text-center">
              <h1 className="text-2xl md:text-3xl font-semibold text-white tracking-tight">
                {login.title}
              </h1>
              <p className="text-sm text-slate-400 mt-1">{login.subtitle}</p>
            </div>
          </div>
          ...
          {/* Footer */}
          <p className="text-slate-500 text-xs text-center">{login.footer}</p>
        </div>
      </div>
    </div>
  );
