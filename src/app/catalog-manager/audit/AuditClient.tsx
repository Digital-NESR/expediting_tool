...
export default function AuditClient({
  ...
  const rows = useMemo(
    () =>
      log.filter(
        (l) =>
          (type === 'All' || l.action === type) &&
          (!q.trim() ||
            `${l.user_name} ${l.target} ${l.detail ?? ''}`.toLowerCase().includes(q.toLowerCase())),
      ),
    [log, type, q],
  );
      ...
      <div className="cm-stagger mx-auto max-w-5xl space-y-5">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Immutable record of every system action · retained 5 years
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select
            ...
            className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-[13px] outline-none focus:border-[#307c4c]"
          >
            {types.map((t) => (
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-2.5">
                      <span
                        className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[11.5px] font-semibold ring-1 ring-inset ring-black/[0.04] ${b.pill}`}
                      >
                    ...
                    <td className="max-w-[340px] px-3 py-2.5 text-slate-500">
                      <span className="block truncate" title={l.detail ?? undefined}>
                        {l.detail}
                      </span>
                    </td>
                    <td className="whitespace-nowrap px-4 py-2.5 text-right font-mono text-[12px] text-slate-400">
                      {l.occurred_at}
                    </td>
                  </tr>
                );
          ...
          {rows.length === 0 && (
            <EmptyState
              ...
              sub="Try a different action type or search term."
            />
          )}
        </div>
      </div>
