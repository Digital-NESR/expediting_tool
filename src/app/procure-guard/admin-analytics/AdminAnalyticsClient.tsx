    ...
    <div className="min-h-[100dvh] bg-slate-50 p-4">
      <div className="mx-auto max-w-xl rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Sign in with admin access to view ProcureGuard usage analytics.
        </p>
        <Link
          href="/procure-guard"
          className="mt-5 inline-flex rounded-md bg-[#307c4c] px-4 py-2 text-sm font-bold text-white hover:bg-[#307c4c]/90"
        >
          Back to dashboard
        </Link>
      </div>
...
function PageMetricsTable({
  ...
  rows: ProcureGuardUsagePageMetric[];
  sortMode: SortMode;
}) {
  const sortedRows = useMemo(
    () =>
      [...rows].sort((a, b) => {
        ...
        if (sortMode === 'time')
          return b.total_duration_ms - a.total_duration_ms || b.views - a.views;
    ...
    <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-100 px-4 py-3">
        ...
        <p className="mt-1 text-xs text-slate-500">
          Where users spend the most time across ProcureGuard.
        </p>
      </div>
            ...
            {sortedRows.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-5 text-center text-slate-500">
                  No page tracking yet. Open a few ProcureGuard pages, then come back here.
                </td>
              </tr>
            ) : (
              sortedRows.map((row) => (
                <tr key={row.path} className="hover:bg-[#307c4c]/5">
                  ...
                  <td className="px-4 py-3 text-right text-slate-600">
                    {formatDuration(row.longest_duration_ms)}
                  </td>
                </tr>
              ))
            )}
...
function ClickMetricsTable({
  rows,
  sortMode,
}: {
  rows: ProcureGuardUsageClickMetric[];
  sortMode: SortMode;
}) {
  const sortedRows = useMemo(
    () =>
      [...rows].sort((a, b) => {
        ...
        if (sortMode === 'time')
          return b.average_click_delay_ms - a.average_click_delay_ms || b.clicks - a.clicks;
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-100 px-4 py-3">
        ...
        <p className="mt-1 text-xs text-slate-500">
          Average time from landing on a page until a user clicks a control.
        </p>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {sortedRows.length === 0 ? (
              ...
              sortedRows.map((row) => (
                <tr
                  key={`${row.path}-${row.target_label}-${row.target_href || row.target_tag}`}
                ...
                >
                  <td className="max-w-[340px] px-4 py-3">
                    ...
                    <p className="truncate text-xs text-slate-500">
                      {row.target_tag}
                      {row.target_href ? ` · ${row.target_href}` : ''}
                    </p>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right font-semibold text-slate-900">
                    {formatDuration(row.average_click_delay_ms)}
                  </td>
                  <td className="px-4 py-3 text-right text-slate-600">
                    {formatDuration(row.slowest_click_delay_ms)}
                  </td>
                </tr>
              ))
            )}
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-100 px-4 py-3">
        ...
        <p className="mt-1 text-xs text-slate-500">
          Per-user activity captured in the last 30 days.
        </p>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
            ...
            {rows.length === 0 ? (
              ...
              rows.map((row) => (
                <tr key={row.user_email} className="hover:bg-[#307c4c]/5">
                  ...
                  <td className="px-4 py-3 text-right text-slate-600">{row.sessions}</td>
                  <td className="px-4 py-3 text-right text-slate-700">
                    {formatDuration(row.average_page_duration_ms)}
                  </td>
                  <td className="px-4 py-3 text-right text-slate-700">
                    {formatDuration(row.average_click_delay_ms)}
                  </td>
...
export default function AdminAnalyticsClient({
  ...
  data: ProcureGuardAdminAnalyticsData | null;
  embedded?: boolean;
}) {
    ...
    <div className={`${embedded ? '' : 'min-h-[100dvh]'} bg-white text-slate-900`}>
      {!embedded && (
        <ProcureGuardSidebar
          ...
          pendingCount={data.pending_review}
          ...
          <div className="mx-auto flex max-w-[1320px] items-center gap-3 px-4 py-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Last 30 days · generated {fmtDateTime(data.generated_at)}
              </p>
            </div>
            <div className="ml-auto flex items-center gap-2">
              <Link
                href="/admin?tool=procureguard-admin"
      ...
      <main className={`${embedded ? '' : 'mx-auto max-w-[1320px] px-4 py-5'}`}>
        <section className="mb-5 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="text-2xl font-black text-slate-950">
              How users move through ProcureGuard
            </h1>
            <p className="mt-1 max-w-2xl text-sm text-slate-500">
              Page time and clicks are captured locally for signed-in ProcureGuard users. Form field
        ...
        <section className="grid grid-cols-1 gap-3 md:grid-cols-3 xl:grid-cols-6">
          <MetricCard
            ...
            detail="Completed page visits"
          />
          <MetricCard
            label="Clicks"
            value={data.summary.clicks}
            detail="Tracked button/link/control clicks"
          />
          ...
          <MetricCard
            label="Avg. Page Time"
            value={formatDuration(data.summary.average_page_duration_ms)}
            detail="Average page visit duration"
          />
          <MetricCard
            label="Avg. To Click"
            value={formatDuration(data.summary.average_click_delay_ms)}
            detail="Average delay before a click"
          />
        </section>
        ...
        <section className="mt-5 rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 px-4 py-3">
            ...
            <p className="mt-1 text-xs text-slate-500">
              Useful for confirming the tracker is firing while testing locally.
            </p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
                ...
                {data.recent_events.length === 0 ? (
                  ...
                  data.recent_events.map((row) => (
                    <tr key={row.id} className="hover:bg-[#307c4c]/5">
                      <td className="px-4 py-3">
                        <span
                          className={`rounded-full px-2 py-1 text-xs font-bold ${row.event_type === 'click' ? 'bg-blue-50 text-blue-700' : 'bg-[#307c4c]/10 text-[#307c4c]'}`}
                        >
                      ...
                      <td className="max-w-[520px] px-4 py-3">
                        <p className="truncate font-semibold text-slate-900">
                          {row.event_type === 'click'
                            ? row.target_text || 'Unknown click'
                            : row.page_title || row.path}
                        </p>
                        <p className="truncate text-xs text-slate-500">
                          {row.target_href || row.path}
                        </p>
                      </td>
                      <td className="px-4 py-3 text-right text-slate-700">
