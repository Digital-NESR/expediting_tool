...
const inputClass =
  'w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800 outline-none transition focus:border-[#307c4c] focus:ring-2 focus:ring-[#307c4c]/20';
    ...
    <div className="min-h-[100dvh] bg-slate-50 p-4">
      <div className="mx-auto max-w-xl rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Sign in with an account that has ProcureGuard admin access.
        </p>
        <Link
          href="/procure-guard"
          className="mt-5 inline-flex rounded-md bg-[#307c4c] px-4 py-2 text-sm font-bold text-white hover:bg-[#307c4c]"
        >
          Back to dashboard
        </Link>
      </div>
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-[0.6875rem] font-semibold whitespace-nowrap ${badge.className}`}
    >
...
function AdhocForm({
  ...
  actor: ProcureGuardAdminData['actor'];
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          disabled={isPending}
          className="rounded-md bg-[#307c4c] px-3 py-2 text-xs font-bold text-white hover:bg-[#307c4c] disabled:opacity-60"
        >
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-4">
        ...
        <Field label="Spend Category">
          <input
            ...
            onChange={(e) => update('spendCategory', e.target.value)}
        ...
        <Field label="Status">
          <select
            className={inputClass}
            value={form.status}
            onChange={(e) => update('status', e.target.value as ProcureGuardStatus)}
          >
            {ADHOC_STATUS_OPTIONS.map((item) => (
              <option key={item} value={item}>
                {formatProcureGuardStatusLabel(item)}
              </option>
            ))}
          </select>
        </Field>
        ...
        <div className="md:col-span-2">
          <Field label="Reason">
            <input
              ...
              onChange={(e) => update('paymentReason', e.target.value)}
              required
            />
          </Field>
        </div>
    ...
    <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-4">
        ...
        <Field label="SAP Vendor ID">
          <input
            className={inputClass}
            value={form.sapVendorId}
            onChange={(e) => update('sapVendorId', e.target.value)}
            required
          />
        </Field>
        <Field label="Amount">
        ...
        <Field label="Status">
          <select
            ...
            onChange={(e) => update('status', e.target.value as ProcureGuardStatus)}
          >
            {ADVANCE_STATUS_OPTIONS.map((item) => (
              <option key={item} value={item}>
                {formatProcureGuardStatusLabel(item)}
              </option>
            ))}
        ...
        <Field label="Credit Limit USD">
          <input
            ...
            onChange={(e) => update('creditLimitUsd', e.target.value)}
            required
          />
        </Field>
        <div className="md:col-span-2">
          <Field label="Purpose">
            <input
              ...
              onChange={(e) => update('advancePurpose', e.target.value)}
              required
            />
          </Field>
        </div>
            ...
            rows.map((row) => (
              <tr key={row.id} className="hover:bg-[#307c4c]/5">
                <td className="px-4 py-3 font-bold text-slate-900">
                  <Link
                    href={`/procure-guard/adhoc-payments/${row.id}`}
                    className="hover:text-[#307c4c] hover:underline"
                  >
                ...
                <td className="px-4 py-3">
                  ...
                  <p className="text-xs text-slate-500">
                    {row.invoice_number || row.po_number || 'No invoice or PO'}
                  </p>
                </td>
          ...
          {rows.length === 0 ? (
            ...
            rows.map((row) => (
              <tr key={row.id} className="hover:bg-[#307c4c]/5">
                <td className="px-4 py-3 font-bold text-slate-900">
                  <Link
                    href={`/procure-guard/advance-payments/${row.id}`}
                    className="hover:text-[#307c4c] hover:underline"
                  >
                    {row.reference_number}
                  </Link>
                ...
                <td className="px-4 py-3">
                  <p className="font-semibold text-slate-900">{row.vendor_name}</p>
                  <p className="text-xs text-slate-500">
                    {row.contract_reference || row.po_number || 'No contract or PO'}
                  </p>
                </td>
                <td className="px-4 py-3 font-semibold">{usdFmt(row.amount, row.currency)}</td>
                <td className="px-4 py-3">
  ...
  return [...groups.entries()]
    .map(([key, groupRows]) => {
      ...
      return {
        ...
        approvalSteps: [
          ...new Set(groupRows.map((row) => row.approval_status || 'General notification')),
        ].sort(),
      };
    })
    ...
    <div>
      ...
      <div className="mt-2 flex flex-wrap gap-1.5">
        {visible.map((value) => (
          <span
            key={value}
            className="rounded-md border border-slate-200 bg-slate-50 px-2 py-1 text-[0.6875rem] font-semibold text-slate-700"
          >
            {value}
          </span>
        ))}
...
function RecipientPersonEditor({
  group,
  onDone,
}: {
  group: RecipientPersonGroup;
  onDone: (message: string) => void;
}) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  ...
  function save() {
    ...
    startTransition(async () => {
      ...
      onDone(
        `Notification recipient saved for ${displayName || email} across ${group.rows.length} assignment${group.rows.length === 1 ? '' : 's'}.`,
      );
      router.refresh();
    });
  }
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(260px,0.8fr)_minmax(0,1.5fr)_auto] xl:items-start">
        <div className="space-y-3">
          <div>
            <p className="text-xs font-bold text-slate-900">
              {group.displayName || 'Unnamed recipient'}
            </p>
          ...
          <div className="rounded-md border border-[#307c4c]/20 bg-[#307c4c]/10 px-3 py-2">
            <p className="text-[0.6875rem] font-bold text-[#307c4c]">
              {group.rows.length} assignment{group.rows.length === 1 ? '' : 's'}
            </p>
            <p className="mt-0.5 text-[0.6875rem] text-slate-600">
              {group.countries.length} countr{group.countries.length === 1 ? 'y' : 'ies'} ·{' '}
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <RecipientChipList
            ...
            values={group.approvalSteps.map(formatProcureGuardStatusLabel)}
          />
        </div>
        ...
        <button
          ...
          onClick={save}
          className="rounded-lg bg-[#307c4c] px-4 py-2 text-xs font-bold text-white shadow-sm hover:bg-[#307c4c]/80 disabled:opacity-60"
        >
...
function NotificationRecipientsPanel({
  ...
  const countries = useMemo(
    () => ['All', ...Array.from(new Set(rows.map((row) => row.country))).sort()],
    [rows],
  );
  const roles = useMemo(
    () => ['All', ...Array.from(new Set(rows.map((row) => row.notification_role))).sort()],
    [rows],
  );
  ...
  function runWebhookTest() {
    ...
    startWebhookTest(async () => {
      ...
      if (result.success && result.data) {
        setWebhookTestResult(
          `Webhook reached ${result.data.webhookHost}${result.data.webhookPath}: ${result.data.status} ${result.data.statusText || 'OK'}`,
        );
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Grouped by person so shared positions like CFO can be changed once across every
              assigned country and approval role.
            </p>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              {webhookTestResult && (
                <span
                  className={`rounded-md border px-3 py-2 text-xs font-semibold ${webhookTestResult.startsWith('Webhook reached') ? 'border-[#307c4c]/20 bg-[#307c4c]/10 text-[#307c4c]' : 'border-red-200 bg-red-50 text-red-700'}`}
                >
                  {webhookTestResult}
                </span>
              )}
      ...
      {searchedGroups.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-4 text-center text-sm text-slate-500 shadow-sm">
          No notification recipients match these filters.
        </div>
      ) : (
        <div className="space-y-3">
          {searchedGroups.map((group) => (
            <RecipientPersonEditor key={group.key} group={group} onDone={onDone} />
          ))}
        </div>
      )}
    </div>
              ...
              <tr key={row.id} className="hover:bg-[#307c4c]/5">
                <td className="px-4 py-3 font-bold text-slate-900">
                  {row.request_type === 'adhoc' || row.request_type === 'advance' ? (
                    <Link
                      href={`/procure-guard/${row.request_type === 'adhoc' ? 'adhoc-payments' : 'advance-payments'}/${row.request_id}`}
                      className="hover:text-[#307c4c] hover:underline"
                    >
                      {row.reference_number}
                    </Link>
                ...
                <td className="px-4 py-3 text-slate-600">
                  {row.actor_email || row.actor_name || 'System'}
                </td>
...
function DelegationsPanel({
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        onDone(
          `Delegated ${delegatorEmail}'s approvals to ${delegateEmail}. Both have been emailed.`,
        );
        setDelegatorEmail('');
        setDelegateEmail('');
        setDelegateName('');
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <form onSubmit={submit} className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <Field label="Approver (delegator)">
            <select
              ...
              onChange={(e) => setDelegatorEmail(e.target.value)}
              required
            >
          ...
          <Field label="Delegate email">
            <input
              ...
              placeholder="colleague@nesr.com"
            />
          </Field>
          ...
          <div className="flex items-end">
            <button
              type="submit"
              disabled={isPending}
              className="rounded-lg bg-[#307c4c] px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-[#307c4c]/85 disabled:opacity-60"
            >
        ...
        {approvers.length === 0 && (
          <p className="mt-3 text-xs text-slate-400">
            No approvers found. Assign approval access from the Access Approvals page first.
          </p>
        )}
      </section>
                ...
                <div key={d.id} className="flex items-center justify-between gap-4 p-4">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span
                        className={`rounded-full px-2 py-0.5 text-[0.6875rem] font-semibold ${live ? 'bg-[#307c4c]/10 text-[#307c4c]' : 'bg-slate-100 text-slate-500'}`}
                      >
...
export default function AdminPanelClient({
  ...
  data: ProcureGuardAdminData | null;
  embedded?: boolean;
}) {
  ...
  const tabItems = [
    {
      ...
      description: 'Update who receives approval notifications for each role and country.',
    },
    {
      key: 'delegations',
      label: 'Delegations',
      count: counts.delegations,
      description: 'Set up and revoke approval delegations on behalf of approvers.',
    },
    {
      key: 'adhoc',
      label: 'Adhoc POs',
      count: counts.adhoc,
      description: 'Review and remove adhoc test records.',
    },
    {
      key: 'advance',
      label: 'Advance Payments',
      count: counts.advance,
      description: 'Review and remove advance test records.',
    },
    {
      key: 'activity',
      label: 'Activity Log',
      count: counts.activity,
      description: 'See meaningful approval activity.',
    },
        ...
        <header className="sticky top-0 z-20 border-b border-gray-100 bg-white/80 backdrop-blur-md">
          <div className="mx-auto flex max-w-[1320px] items-center gap-3 px-4 py-3">
            ...
            <div className="ml-auto flex items-center gap-2">
              <Link
                href="/procure-guard"
                className="rounded-md border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 hover:bg-[#307c4c]/5"
              >
                Dashboard
              </Link>
              <Link
                href="/procure-guard/analytics"
                className="rounded-md border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 hover:bg-[#307c4c]/5"
              >
                Analytics
              </Link>
      ...
      <main className={`${embedded ? '' : 'mx-auto max-w-[1320px] px-4 py-5'}`}>
        ...
        <section className="mt-6">
          <div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-sm font-black uppercase tracking-wide text-[#307c4c]">
                Admin Controls
              </p>
              <h2 className="text-xl font-black text-slate-950">Data and notification routing</h2>
            </div>
            <p className="max-w-xl text-sm text-slate-500">
              Use the tiles below to manage ProcureGuard records, approval notifications, and
              delegations.
            </p>
          </div>
          ...
          <div className="mt-5">
            ...
            {tab === 'recipients' && (
              <NotificationRecipientsPanel rows={data.notification_recipients} onDone={setNotice} />
            )}
            {tab === 'delegations' && (
              <DelegationsPanel
