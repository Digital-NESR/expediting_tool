...
export default function ProcureGuardDelegateClient({ data }: { data: ProcureGuardDelegationData }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await grantProcureGuardDelegation({
        delegateEmail: email,
        delegateName: name,
  ...
  return (
    <div className="min-h-[100dvh] bg-slate-50 font-sans text-slate-900">
      ...
      <main className="mx-auto max-w-[900px] space-y-4 px-4 py-4 sm:px-6">
        <ProcureGuardHero
          title="Delegation"
          subtitle="Hand your approval authority to a colleague while you're away. You both keep the ability to act, and you can revoke any time."
        />

        {banner && (
          <div className="rounded-lg border border-[#307c4c]/20 bg-[#307c4c]/10 px-4 py-3 text-sm font-semibold text-[#307c4c]">
            {banner}
          </div>
        )}
        {error && (
        ...
        {canDelegate ? (
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              The delegate is identified by their NESR sign-in email and inherits your approval
              scope.
            </p>
            <form onSubmit={submit} className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="sm:col-span-2">
                ...
                <p className="mt-1 text-xs text-slate-400">
                  Pick from the employee directory to fill the fields below, or type them in
                  manually.
                </p>
              </div>
              <div>
                <label className={LBL}>
                  <span className="mr-1 text-red-500">*</span>Delegate email
                </label>
                <input
                  type="email"
                  required
              ...
              <div>
                <label className={LBL}>
                  Delegate name <span className="font-normal text-slate-400">(optional)</span>
                </label>
                <input
                  className={INP}
                  value={name}
              ...
              <div>
                ...
                />
                <p className="mt-1 text-xs text-slate-400">
                  Leave blank to keep active until you revoke it.
                </p>
              </div>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-500 shadow-sm">
            You don&apos;t have approval authority of your own to delegate. This page shows any
            authority delegated to you below.
          </section>
        )}
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 px-5 py-4">
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              People who can currently act on your behalf.
            </p>
          </div>
          {data.granted.length === 0 ? (
            <div className="p-4 text-center text-sm text-slate-500">
              You haven&apos;t delegated to anyone.
            </div>
          ) : (
        ...
        {data.received.length > 0 && (
          <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="border-b border-slate-100 px-5 py-4">
              ...
              <p className="mt-0.5 text-xs text-slate-500">
                Approvers whose requests you can currently act on.
              </p>
            </div>
            <div className="divide-y divide-slate-100">
              {data.received.map((d) => (
