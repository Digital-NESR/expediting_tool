...
export default function PirCatalogClient({
  ...
  async function exportCsv() {
    ...
    try {
      ...
      const body = all
        .map((e) => FIELDS.map((f) => `"${csvSafe(e[f.key] ?? '').replace(/"/g, '""')}"`).join(','))
        .join('\n');
  ...
  const activeFilters =
    [filters.country, filters.porg, filters.plant, filters.mgroup].filter(Boolean).length +
    (filters.q ? 1 : 0);
  const maxPage = Math.max(1, Math.ceil(total / pageSize));
  const cur = Math.min(page, maxPage);
  const selectCls =
    'rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-[13px] text-slate-700 outline-none focus:border-[#307c4c] focus:ring-2 focus:ring-[#307c4c]/20';
    ...
    <CatalogManagerShell
      ...
      headerAction={
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-600 shadow-sm transition-all hover:border-[#6aaf8e] hover:text-slate-900 active:scale-[0.98] disabled:opacity-50"
        >
          {exporting ? (
    ...
    >
      <div className="cm-stagger space-y-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h1 className="text-xl font-bold tracking-tight text-slate-900">
              PIR / Inventory catalog
            </h1>
            <p className="mt-1 text-[13px] text-slate-500">
              Read-only mirror of SAP Purchasing Info Records — synced from Power BI. View and
              export only; nothing here can be edited.
            </p>
          </div>
        ...
        <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <StatCard
            label="Plants"
            value={stats.plants.toLocaleString()}
            sub="Distinct plants"
            icon="globe"
            tone="ink"
          />
        ...
        <Card className="flex min-h-0 flex-col">
          <div className="shrink-0 border-b border-slate-100 p-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <select
                value={filters.porg}
                onChange={(e) => pushParams({ porg: e.target.value })}
                className={selectCls}
              >
              ...
              <select
                value={filters.plant}
                onChange={(e) => pushParams({ plant: e.target.value })}
                className={selectCls}
              >
              ...
              <select
                value={filters.mgroup}
                onChange={(e) => pushParams({ mgroup: e.target.value })}
                className={selectCls}
              >
                <option value="">All material groups</option>
                {facets.mgroups.map((c) => (
                  <option key={c} value={c}>
                    {c}
                  </option>
                ))}
              ...
              {activeFilters > 0 && (
                <button
                  onClick={clearAll}
                  className="text-[12.5px] font-semibold text-[#1d4f31] hover:underline"
                >
                  Clear ({activeFilters})
                </button>
              )}
            ...
            <p className="mt-2.5 flex items-center gap-2 text-[12.5px] text-slate-500">
              <span>
                ...
                {total !== stats.total ? ` of ${stats.total.toLocaleString()}` : ''}
              </span>
          ...
          {total === 0 ? (
            <EmptyState
              ...
              sub={
                stats.total === 0
                  ? 'The catalog fills once the n8n sync runs its first load.'
            ...
            <>
              <div
                className={`${tableClasses.scroller} ${isPending ? 'opacity-60 transition-opacity' : ''}`}
              >
                <table className={`${tableClasses.table} text-[13px]`}>
                  ...
                  <tbody className={tableClasses.tbody}>
                    {rows.map((e, i) => (
                      ...
                      >
                        <td className={`${tableClasses.td} font-mono text-[12px] text-slate-500`}>
                          {e.info_record_number ?? '—'}
                        </td>
                        <td className={`${tableClasses.td} max-w-[280px]`}>
                          <div className="truncate font-semibold text-slate-900">
                            {e.material_description || e.product_number || '—'}
                          </div>
                          <div className="truncate font-mono text-[11px] text-slate-400">
                            {e.product_number}
                            {e.material_group ? ` · ${e.material_group}` : ''}
                          </div>
                        </td>
                        <td className={`${tableClasses.td} max-w-[200px]`}>
                          ...
                          <div className="font-mono text-[11px] text-slate-400">
                            {e.suppliers_account_number}
                          </div>
                        </td>
                        <td className={`${tableClasses.td} whitespace-nowrap text-slate-500`}>
                          {e.purchasing_organization ?? '—'}
                          {e.plant ? ` · ${e.plant}` : ''}
                        </td>
                        <td className={`${tableClasses.td} whitespace-nowrap text-slate-500`}>
                          {e.country ?? '—'}
                        ...
                        <td
                          className={`${tableClasses.td} whitespace-nowrap text-[12.5px] text-slate-500`}
                        >
                          {e.order_unit ?? e.base_unit_of_measure ?? '—'}
                        </td>
                        <td
                          className={`${tableClasses.td} whitespace-nowrap text-right font-mono font-semibold text-slate-900`}
                        >
                          {money(e.unit_price, e.currency_key)}
                        </td>
                        <td className={`${tableClasses.td} whitespace-nowrap text-slate-500`}>
                          {e.incoterms ?? '—'}
                          {e.incoterms_location_1 ? ` · ${e.incoterms_location_1}` : ''}
                        </td>
                        <td
                          className={`${tableClasses.td} whitespace-nowrap text-[12.5px] text-slate-500`}
                        >
                          {e.valid_till_expiry_date ?? '—'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              {maxPage > 1 && (
                <div className="flex items-center justify-between gap-3 border-t border-slate-100 px-4 py-3 text-[12.5px] text-slate-500">
                  <span>
                    Showing {(cur - 1) * pageSize + 1}–{Math.min(cur * pageSize, total)} of{' '}
                    {total.toLocaleString()}
                  </span>
                  <div className="flex items-center gap-1.5">
                    <button
                      onClick={() => pushParams({ page: cur - 1 }, false)}
                      disabled={cur <= 1 || isPending}
                      className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 font-semibold text-slate-600 transition-colors hover:border-[#6aaf8e] disabled:opacity-40"
                    >
                    ...
                    <button
                      onClick={() => pushParams({ page: cur + 1 }, false)}
                      disabled={cur >= maxPage || isPending}
                      className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 font-semibold text-slate-600 transition-colors hover:border-[#6aaf8e] disabled:opacity-40"
                    >
      ...
      {detail &&
        createPortal(
          <div className="fixed inset-0 z-[90] flex items-start justify-center p-4 pt-[8vh]">
            <button
              aria-label="Close"
              className="cm-fade-in absolute inset-0 bg-slate-950/40 backdrop-blur-sm"
              ...
              <div className="flex items-start justify-between gap-3 border-b border-slate-100 bg-gradient-to-r from-[#eaf4ef] to-white px-5 py-4">
                <div className="min-w-0">
                  <div className="mb-1 flex items-center gap-2">
                    <span className="rounded-md border border-slate-200 bg-white px-2 py-0.5 font-mono text-[12px] text-slate-600">
                      {detail.info_record_number ?? '—'}
                    </span>
                    {detail.status && (
                      <span className="rounded-full bg-[#307c4c]/10 px-2 py-0.5 text-[11px] font-semibold text-[#1d4f31]">
                        {detail.status}
                      </span>
                    )}
                  </div>
                        ...
                        {items.map((f) => (
                          <div key={String(f.key)} className="min-w-0">
                            ...
                            <div
                              ...
                              title={fmt(detail[f.key])}
                            >
                              {f.key === 'unit_price'
                                ? money(detail.unit_price, detail.currency_key)
                                : fmt(detail[f.key])}
                            </div>
                          </div>
                        ))}
