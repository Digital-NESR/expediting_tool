    ...
    <div style={{ animation: 'fadeIn 0.2s ease' }}>
      ...
      >
        {vm.kpiCards.map((kpi) => (
          <div key={kpi.label} style={kpi.cardStyle}>
            ...
            <div style={{ fontSize: 10, color: '#58595B', marginTop: 3, lineHeight: 1.3 }}>
              {kpi.sub}
            </div>
          </div>
        ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '3fr 1fr', gap: 12, marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', borderRadius: 7, overflow: 'hidden' }}>
            {vm.pipeline.map((step) => (
              <div key={step.id} style={step.stepStyle}>
                ...
                <div style={{ fontSize: 11, fontWeight: 'bold', marginTop: 4, lineHeight: 1.2 }}>
                  {step.label}
                </div>
                {step.active && (
                  <div style={{ fontSize: 10, marginTop: 2, opacity: 0.85 }}>{step.sub}</div>
                )}
              </div>
            ))}
          </div>
        ...
        >
          ...
          <div style={{ fontSize: 10, color: '#58595B', marginBottom: 8 }}>
            of total 18-month PO balance
          </div>
          <div
            style={{
              background: '#E0E8E3',
