...
function EditableApproverMatrix({
      ...
      prev
        ? {
            ...
            cells: {
              ...
              [country]: {
                ...prev.cells[country],
                [col.key]: { name: emp.name, email: emp.email },
              },
            },
          }
        : prev,
  ...
  return (
    <>
      {error && (
        <p className="mb-2 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700">
          {error}
        </p>
      )}
                ...
                {data.columns.map((col) => (
                  <th key={col.key} className="px-3 py-3">
                    <ColumnHeader
                      ...
                      onAssign={(emp) => assignColumn(col, emp)}
                    />
                  </th>
                ))}
              </tr>
...
function CountryScopePicker({
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-2">
      ...
      <div
        className={`mt-1 grid max-h-40 grid-cols-2 gap-x-2 overflow-auto border-t border-slate-100 pt-1 ${all ? 'pointer-events-none opacity-40' : ''}`}
      >
        {COUNTRY_OPTIONS.map((c) => (
          <label
            key={c}
            className="flex items-center gap-2 rounded-md px-2 py-1 text-[13px] text-slate-600 hover:bg-slate-50"
          >
            <input
              type="checkbox"
              ...
              className="h-3.5 w-3.5 accent-[#307c4c]"
            />
  ...
  return (
    <section className="mt-8">
      ...
      <p className="mb-3 text-[12px] text-slate-400">
        Give someone read-only visibility of requests and analytics, scoped to specific countries or
        all countries. Revoking drops them back to Requester.
      </p>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <CountryScopePicker
              all={all}
              selected={selected}
              onToggleAll={() => setAll((a) => !a)}
              onToggleCountry={(c) => toggleInSet(setSelected, c)}
            />
          </div>
        </div>
              ...
              <div key={g.email} className="px-4 py-3">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex min-w-0 items-center gap-2.5">
                    <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#307c4c]/10 text-[10px] font-bold text-[#307c4c]">
                  ...
                  <div className="flex shrink-0 items-center gap-1.5">
                    <button
                      ...
                      onClick={() => {
                        ...
                        setEditAll(g.countries.length === 0);
                        setEditSelected(new Set(g.countries));
                      }}
                ...
                {editEmail === g.email ? (
                  <div className="mt-3">
                    <CountryScopePicker
                      ...
                      onToggleAll={() => setEditAll((a) => !a)}
                      onToggleCountry={(c) => toggleInSet(setEditSelected, c)}
                    />
                    <div className="mt-2 flex justify-end gap-2">
                      <button
                        type="button"
                        onClick={() => setEditEmail(null)}
                        className="rounded-md px-3 py-1.5 text-xs font-medium text-slate-500 hover:bg-slate-50"
                      >
                        Cancel
                      </button>
                      <button
                        type="button"
                        disabled={saving || (!editAll && editSelected.size === 0)}
                        onClick={() => saveEdit(g)}
                        className="rounded-md bg-[#307c4c] px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-50"
                      >
                        Save
                      </button>
                    </div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {g.countries.length === 0 ? (
                      <span className="rounded-full bg-[#307c4c]/10 px-2 py-0.5 text-[11px] font-semibold text-[#307c4c]">
                        All countries
                      </span>
                    ) : (
                      g.countries.map((c) => (
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-[12px] text-slate-400">
        The approver notified for each role in each country. Supply Chain Director is split into
        Adhoc and Advance. Click any cell to reassign from the employee directory.
      </p>
