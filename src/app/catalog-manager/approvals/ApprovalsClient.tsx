        ...
        {pending.length === 0 ? (
          ...
          <div className="space-y-4">
            {groups.map(({ supplier, list }) => {
              ...
              return (
                <div key={supplier} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                  <div className="flex items-center gap-3 px-4 py-4">
                    <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#307c4c]/10 text-[#1d4f31]"><Icon name="building" className="h-5 w-5" /></span>
                    ...
                    {actionable.length > 1 && (
                      <button onClick={() => setBulkDialog({ supplier, entries: actionable })} className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-[#307c4c]/30 bg-white px-3 py-1.5 text-[12.5px] font-semibold text-[#1d4f31] hover:bg-[#307c4c]/5">
                        <Icon name="approve" className="h-4 w-4" /> Approve all {actionable.length}
                      </button>
                    )}
                  </div>
                  {list.map((e) => (
                    <div key={e.id} className="flex items-center gap-3 border-t border-slate-100 px-4 py-3">
                      ...
                      {canActOn(e) && (
                        <div className="flex shrink-0 gap-2">
                          <button onClick={() => setDialog({ entry: e, decision: 'reject' })} title="Reject / revise" className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-red-200 text-red-600 hover:bg-red-50"><Icon name="x" className="h-4 w-4" /></button>
                          <button onClick={() => setDialog({ entry: e, decision: 'approve' })} className="inline-flex items-center gap-1.5 rounded-lg bg-[#307c4c] px-3 py-1.5 text-[12.5px] font-semibold text-white hover:bg-[#2b6f44]"><Icon name="check" className="h-4 w-4" /> Approve</button>
                        </div>
                      )}
                    </div>
                  ))}
