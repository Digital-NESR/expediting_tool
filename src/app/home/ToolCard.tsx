...
const CARD_BASE = 'group relative flex w-full flex-col gap-4 rounded-xl border border-gray-200 bg-white p-8 text-left transition-all duration-200';
...
export function ToolCard({
  ...
  const canOpen = tool.access.kind === 'status'
  ...
  if (canOpen) {
    ...
    if (status === 'new') {
      ...
      action = { label: 'Reapply for access →', className: 'text-xs font-semibold text-red-600 group-hover:underline' };
    }
  }

  const toneClass = tool.tone === 'live'
  ...
  return (
    <div className={`${CARD_BASE} ${toneClass}`}>
      ...
      {interactive ? (
        tool.external ? (
          <a href={tool.route} title="Open in this tab" className={logoClass} style={tool.logoStyle}>
            {tool.icon}
          </a>
        ) : (
          <button
        ...
        <div className={`relative z-10 flex h-12 w-12 items-center justify-center rounded-xl ${tool.logoClass}`} style={tool.logoStyle}>
          {tool.icon}
        </div>
      )}
      ...
      <div className="relative z-10 flex-1 pointer-events-none">
        {tool.pill ? (
          <div className="flex flex-wrap items-center gap-2">
            <h3 className={tool.tone === 'live' ? 'text-[18px] font-semibold text-slate-900' : 'text-[18px] font-semibold text-gray-500'}>{tool.name}</h3>
        ...
        {tool.subtitle && <p className="mt-0.5 text-[13px] font-medium text-slate-400">{tool.subtitle}</p>}
      ...
      <div className="relative z-10 mt-auto flex items-center justify-between pointer-events-none">
        {tool.badge.kind === 'status' ? <AccessBadge status={status} isAdmin={isAdmin} />
          : tool.badge.kind === 'procureGuard' ? <ProcureGuardBadge accessType={procureGuardAccessType} />
          : tool.badge.kind === 'success' ? <TickBadge label={tool.badge.label} />
          : tool.badge.kind === 'lock' ? <LockBadge label={tool.badge.label} />
          : (
            <span className="rounded-full bg-gray-100 px-2.5 py-1 text-[11px] font-medium text-gray-400">
              {canOpen ? 'Admin Preview' : 'Coming Soon'}
            </span>
          )}
        {action && <span className={action.className} style={action.style}>{action.label}</span>}
      </div>
