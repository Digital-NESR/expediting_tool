...
function StatusPill({ level }: { level: string }) {
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[11px] font-semibold whitespace-nowrap ${ALERT_PILL[level] || ''}`}
    >
...
function UpdateStatusModal({
  ...
  const INP =
    'w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#006B0C]/20 focus:border-[#006B0C] bg-white';
  ...
  async function handleSubmit() {
    ...
    try {
      const result = await updateShipmentStatus({
        ...
        refundAmountUsd:
          selectedStatus === 'Closed - Refund Recovered' && refundAmount
            ? Number(refundAmount)
            : null,
        ...
        justification:
          selectedStatus === 'Closed - Refund Recovered' && isDifferent
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xl w-full max-w-lg overflow-y-auto max-h-[90vh]">
        ...
        <div className="px-6 py-5 border-b border-slate-100">
          ...
          <p className="text-[12.5px] text-slate-500 mt-1 flex items-center gap-1.5">
            ...
            <span
              className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[10.5px] font-semibold ${sb.className}`}
            >
              {sb.label}
            </span>
          </p>
            ...
            <>
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-1">
                  Updated re-export deadline after extension approval
                </p>
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1.5">
            ...
            <>
              ...
              {isDifferent && (
                <div>
                  ...
                  >
                    ⚠️ The refund amount ({usdFmt(refundNum)}) differs from the original deposit (
                    {usdFmt(depositUsd)}). Please provide a justification.
                  </div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">
...
function StageDocSection({ stageKey, docs }: { stageKey: string; docs: ShipmentDocument[] }) {
...
function StageUploadCard({
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div className="p-5">
        <DocumentUploadSection stage={stage} shipmentId={shipmentId} initialDocuments={docs} />
      </div>
    </div>
  );
}
...
export default function ShipmentDetailClient({
      ...
      <div className="min-h-[100dvh] flex items-center justify-center bg-slate-50 p-6">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-10 text-center max-w-sm">
          ...
          <p className="text-sm text-slate-500 mb-4">
            It may have been removed or the database is unavailable.
          </p>
          <button
            className="text-sm font-medium text-[#006B0C] hover:underline"
  ...
  const totalDays =
    importDate && effDate ? Math.floor((effDate.getTime() - importDate.getTime()) / 86400000) : 1;
  ...
  const checksAll = [
    ...
    { done: !!s.awb_number, text: 'Airway bill / Bill of lading on file' },
    {
        ...
        (Number(s.deposit_usd) || 0) > 0 ||
        (s.movement_type || '').toLowerCase().includes('export'),
      text: 'Customs deposit recorded',
    },
      ...
      <main className="max-w-[1500px] mx-auto px-6 pb-16 pt-6">
          ...
          <div>
            <p className="text-xs text-slate-400 mb-1">
              ...
              <span className="font-mono">
                {s.reference_number || `#${String(s.id).padStart(3, '0')}`}
              </span>
            </p>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2">
              ...
              {s.movement_type && (
                <span
                  className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold ${s.movement_type.toLowerCase().includes('export') ? 'bg-blue-100 text-blue-700 border border-blue-200' : 'bg-cyan-100 text-cyan-700 border border-cyan-200'}`}
                >
              ...
              {(() => {
                ...
                return (
                  <span
                    className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold ${sb.className}`}
                  >
                    {sb.label}
                  </span>
                );
              ...
              <span className="text-xs text-slate-400">
                Logged {fmtDate(s.import_date)} by {s.created_by || '—'}
              </span>
            </div>
          </div>
        ...
        {s.alert_level === 'overdue' && (
          <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-5 text-sm text-red-800">
            ...
            <span>
              <strong>Past re-export deadline by {days !== null ? -days : '?'} days.</strong>{' '}
              Customs deposit ({usdFmt(s.deposit_usd)}) at risk. Contact customs broker and legal
              immediately.
            </span>
          </div>
        )}
        {s.alert_level === 'urgent' && (
          <div className="flex items-center gap-3 bg-orange-50 border border-orange-200 rounded-xl px-4 py-3 mb-5 text-sm text-orange-800">
            ...
            <span>
              <strong>{days} days remaining.</strong> Submit extension request or initiate re-export
              now.
            </span>
          </div>
        )}
          ...
          <div className="space-y-5">
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
                  ...
                  <div className="p-5 grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-4">
                    <Field
                      label="Reference"
                      value={
                        <span className="font-mono text-[12.5px]">{s.reference_number || '—'}</span>
                      }
                    />
                    ...
                    <Field
                      ...
                      value={
                        <span className="font-mono text-[12.5px]">{s.invoice_number || '—'}</span>
                      }
                    />
                    <Field
                      label="Invoice value"
                      value={
                        s.invoice_value_usd != null ? (
                          <span className="tabular-nums">{usdFmt(s.invoice_value_usd)}</span>
                        ) : (
                          '—'
                        )
                      }
                    ...
                    <Field
                      label="PO number"
                      value={<span className="font-mono text-[12.5px]">{s.po_number || '—'}</span>}
                    />
                    <Field
                      label="Customs Ref. No."
                      value={
                        <span className="font-mono text-[12.5px]">
                          {s.customs_reference_number || '—'}
                        </span>
                      }
                    />
                    <Field
                      label="AWB / BL"
                      value={<span className="font-mono text-[12.5px]">{s.awb_number || '—'}</span>}
                    />
                    <Field label="Import date" value={fmtDate(s.import_date)} />
                  </div>
                  {s.comments && (
                    <div className="px-5 pb-5 pt-0 border-t border-slate-100">
                      <p className="text-[10.5px] font-semibold uppercase tracking-wider text-slate-400 mb-1 mt-4">
                        Comments
                      </p>
                  ...
                  {s.customs_docs_location && (
                    <div
                      className={`px-5 pb-5 pt-0 ${s.comments ? '' : 'border-t border-slate-100'}`}
                    >
                      <p className="text-[10.5px] font-semibold uppercase tracking-wider text-slate-400 mb-1 mt-4">
                        Customs Docs Location
                      </p>
                          ...
                          <Field
                            ...
                            value={
                              s.status === 'Closed - Refund Recovered' ? (
                                <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-green-100 text-green-700 border border-green-200">
                                  <span className="w-1.5 h-1.5 rounded-full bg-green-500" />
                                  Refund recovered
                                </span>
                              ) : s.status === 'Closed' ? (
                                <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-gray-100 text-gray-600 border border-gray-200">
                                  <span className="w-1.5 h-1.5 rounded-full bg-gray-400" />
                                  File closed
                                </span>
                              ) : (
                                <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-amber-100 text-amber-700 border border-amber-200">
            ...
            {tab === 'documents' && (
              <div className="space-y-4">
                ...
                {showExtensionDocs && (
                  <StageUploadCard stage="extension" shipmentId={s.id} docs={extensionDocs} />
                )}
                {showClosureDocs && (
                  <StageUploadCard stage="closure" shipmentId={s.id} docs={closureDocs} />
                )}
                {showRefundDocs && (
                  <StageUploadCard stage="refund" shipmentId={s.id} docs={refundDocs} />
                )}
              </div>
            )}
                      ...
                      {activityLog.map((entry) => (
                        <li key={entry.id} className="ml-5">
                          <div className="absolute -left-1.5 mt-1.5 flex items-center justify-center">
                            <span
                              className={`w-2.5 h-2.5 rounded-full ${ACTION_DOT[entry.action] ?? 'bg-slate-300'}`}
                            />
                          </div>
                          <div className="bg-slate-50 rounded-lg border border-slate-100 px-4 py-3">
                            ...
                            {entry.details && (
                              <div className="mt-0.5">
                                {entry.details.split('\n').map((line, i) => (
                                  <p key={i} className="text-[12.5px] text-slate-500">
                                    {line}
                                  </p>
                                ))}
                              </div>
            ...
            {tab === 'compliance' && (
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
                <div className="px-5 py-3.5 border-b border-slate-100">
                  <h2 className="text-sm font-bold text-slate-900">Compliance checklist</h2>
                </div>
                <div className="p-5 space-y-0">
                  {checksAll.map((c, i) => (
                    <div key={i} className="flex items-center gap-3 py-3 border-b border-slate-50">
                      <span
                        className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${c.done ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-400'}`}
                      >
                        {c.done ? (
                          <svg
                            className="w-3 h-3"
                      ...
                      <span
                        className={`text-[13.5px] ${c.done ? 'text-slate-800' : 'text-slate-500'}`}
                      >
                        {c.text}
                      </span>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
              <div className="px-4 py-3 border-b border-slate-100">
                <h3 className="text-sm font-bold text-slate-900">Key dates</h3>
              </div>
              <div className="p-4 space-y-3">
                {[
                  {
                    label: 'Imported',
                    date: fmtDate(s.import_date),
                    struck: false,
                    highlight: false,
                  },
                  {
                    label: 'Original expiry',
                    date: fmtDate(s.expiry_date),
                    struck: !!s.extended_date,
                    highlight: false,
                  },
                  ...(s.extended_date
                    ? [
                  ...
                  <div key={i} className="flex items-center gap-2 text-sm">
                    ...
                    <div className="flex-1" />
                    <span
                      className={`tabular-nums text-[12.5px] ${kd.highlight ? 'font-bold text-[#006B0C]' : 'font-medium text-slate-700'} ${kd.struck ? 'line-through text-slate-400' : ''}`}
                    >
                      {kd.date}
                    </span>
                  </div>
                ...
                {NOTIFICATION_MILESTONES.map(({ label, dbe }) => {
                  ...
                  return (
                    <div key={dbe} className="flex items-center gap-2 text-sm">
                      <svg
                        className={`w-3.5 h-3.5 shrink-0 ${BELL_CLASS[state]}`}
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        strokeWidth={2}
                      ...
                      <div className="flex-1" />
                      <span
                        className={`text-[10.5px] font-semibold px-2 py-0.5 rounded-full ${BADGE_CLASS[state]}`}
                      >
                        {BADGE_LABEL[state]}
                      </span>
                    </div>
