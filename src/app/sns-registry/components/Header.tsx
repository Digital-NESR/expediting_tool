...
export default function Header({ app }: { app: RegistryApp }) {
  ...
  const countryLabel =
    viewer.countryCodes.length === 0
      ? 'All countries'
      : viewer.countryCodes
          .map((code) => app.countries.find((c) => c[1] === code)?.[0] ?? code)
          .join(', ');
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <div style={{ fontSize: 14, fontWeight: 'bold', letterSpacing: 0.3 }}>
              Single &amp; Sole Source Registry
            </div>
            <div style={{ fontSize: 10, color: '#C5E0D2', letterSpacing: 0.5 }}>
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2, alignItems: 'flex-end' }}>
            <div style={{ fontSize: 12.5, fontWeight: 'bold', whiteSpace: 'nowrap' }}>
              {viewer.name}
            </div>
            <div
              style={{ fontSize: 10.5, color: '#C5E0D2', letterSpacing: 0.3, whiteSpace: 'nowrap' }}
            >
      ...
      >
        {nav.map((n) => {
          const active =
            app.screen === n.screen || (app.screen === 'detail' && n.screen === 'registry');
