...
export default function MappingsClient({
  ...
  const [gaps, setGaps] = useState<{ missing: number; noPreferred: number }>({
    missing: 0,
    noPreferred: 0,
  });
  ...
  if (!country) {
    return (
      <div className="mx-auto max-w-[980px] px-6 py-16 text-center text-slate-500">
        No countries available to manage.
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-[1200px] px-6 py-7 lg:px-8">
      <div className="mb-5">
        ...
        <p className="mt-2 max-w-[620px] text-[15px] leading-relaxed text-slate-500">
          Amend the preferred and backup supplier mappings for your country. Promote, demote, add or
          deactivate suppliers. Every change is audit-logged.
        </p>
      </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <div className="text-[12px] text-slate-500">
              Champion: {c?.champion || 'Unassigned'}
            </div>
          </div>
        </div>
        ...
        <div className="min-w-0">
                  ...
                  <span
                    ...
                    style={
                      on
                        ...
                        : { background: '#eef0ef', color: '#58595B' }
                    }
                  >
                    {t.n}
                  </span>
          ...
          <div className="mb-4 flex items-center gap-2.5 rounded-xl border border-slate-200 bg-white px-4 py-2.5 focus-within:border-[#6AAF8E]">
            ...
            <input
              ...
              placeholder={
                mode === 'mapped'
                  ? 'Find a commodity to amend (search the full catalogue)…'
                  : 'Filter within these results…'
              }
          ...
          <div className="mb-3.5 text-[12.5px] text-slate-500">
            {q.trim() ? (
              ...
              <>
                Commodities in the catalogue <b className="text-slate-900">not yet mapped</b> in{' '}
                {c?.name}. Add a supplier to fill the gap.
              </>
            ) : mode === 'no-preferred' ? (
              <>
                Commodities in {c?.name} that have a{' '}
                <b className="text-slate-900">backup but no preferred</b> supplier.
              </>
            ) : (
              <>
                Showing the <b className="text-slate-900">{list.length}</b> commodities currently
                mapped in {c?.name}.
              </>
            )}
          </div>
          ...
          {loading && list.length === 0 && (
            <div className="py-10 text-center text-[13.5px] text-slate-400">Loading…</div>
          )}
      ...
      <div className="flex flex-wrap items-start justify-between gap-3.5">
        <div className="min-w-0">
          ...
          <div className="text-[15px] font-semibold">
            {com.name}
            {com.code && (
              <span className="ml-1.5 font-mono text-[11px]" style={{ color: SG_BRAND }}>
                {com.code}
              </span>
            )}
          </div>
...
function ChipRow({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2.5">
      <span className="w-[70px] font-mono text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-400">
        {label}
      </span>
      {chips.map((m) => (
        <Chip key={m.id} m={m} onChange={onChange} />
      ))}
    </div>
  );
}
...
function Chip({ m, onChange }: { m: SgMapping; onChange: (msg?: string) => void }) {
  ...
  return (
    <div
      ...
      style={
        isPref
          ? { borderColor: '#6AAF8E', background: '#eaf4ef' }
    ...
    >
      ...
      <button
        disabled={busy}
        onClick={flip}
        title={isPref ? 'Make backup' : 'Make preferred'}
    ...
    <div className="relative flex flex-wrap items-center gap-2">
      <div className="flex gap-1 rounded-full bg-[#eef0ef] p-0.5">
        {(['Preferred', 'Backup'] as Tier[]).map((t) => (
          <button
            ...
            style={
              tier === t
                ? { background: '#fff', color: '#1f5d3a', fontWeight: 600 }
                : { color: '#58595B' }
            }
          >
            {t}
      ...
      <div className="relative min-w-[200px] flex-1">
        <input
          autoFocus
          value={text}
          placeholder="Search the Approved Vendor List…"
          ...
          <div className="absolute left-0 right-0 top-[calc(100%+4px)] z-20 max-h-60 overflow-auto rounded-xl border border-slate-200 bg-white p-1.5 shadow-2xl">
            {opts.map((s) => (
              <div
                ...
                className="flex cursor-pointer items-center gap-2.5 rounded-lg px-2.5 py-2 hover:bg-[#eaf4ef]"
              >
                <span
                ...
                {s.countries.includes(country) && (
                  <span className="rounded-full bg-[#C5E0D2] px-2 py-0.5 text-[10px] font-semibold text-[#1f5d3a]">
                    in country
                  </span>
                )}
              </div>
            ...
            {text.trim().length > 0 && opts.length === 0 && (
              <div className="px-2.5 py-3 text-center text-[12px] text-slate-400">
                No matching vendor in the AVL.
              </div>
            )}
          </div>
        )}
      ...
      <button
        disabled={busy}
        onMouseDown={() => submit()}
        className="rounded-full px-3.5 py-2 text-[12.5px] font-semibold text-white disabled:opacity-50"
        style={{ background: SG_BRAND }}
      >
        Add
      </button>
...
function AuditPanel({ items }: { items: SgActivityEntry[] }) {
  ...
  return (
    <div className="sticky top-[88px] rounded-2xl border border-slate-200 bg-white px-5 py-4">
      ...
      <p className="mb-3.5 text-[12px] text-slate-500">
        Every amendment is recorded with user, time and delta.
      </p>
      {items.length === 0 ? (
        ...
        <div className="flex flex-col">
          {items.map((a, i) => (
            <div
              key={a.id}
              className={`flex gap-2.5 py-2.5 ${i < items.length - 1 ? 'border-b border-slate-100' : ''}`}
            >
              <span
                className="mt-1.5 h-2 w-2 shrink-0 rounded-full"
                style={{ background: colors[a.action] ?? '#58595B' }}
              />
              <div className="min-w-0">
                <div className="text-[12.5px] font-medium">
                  {a.action} <span className="font-normal text-slate-500">· {a.details}</span>
                </div>
                <div className="mt-0.5 font-mono text-[10.5px] text-slate-400">
                  {a.performedBy} · {ago(a.performedAt)}
                </div>
              </div>
            </div>
          ))}
