...
function aggregate(requests: ProcureGuardAnalyticsRequest[]): ScopedAnalytics {
  ...
  for (const r of requests) {
    ...
    if (r.request_type === 'adhoc') {
      v.adhoc_count += 1;
      v.adhoc_amount += r.amount_usd;
    } else {
      v.advance_count += 1;
      v.advance_amount += r.amount_usd;
    }
    vendors.set(vk, v);
    ...
    if (r.request_type === 'adhoc') {
      m.adhoc_count += 1;
      m.adhoc_amount += r.amount_usd;
    } else {
      m.advance_count += 1;
      m.advance_amount += r.amount_usd;
    }
    m.total_count += 1;
  ...
  const topVendor =
    [...vendorList].sort((a, b) => b.count - a.count || b.amount - a.amount)[0] ?? null;
...
type SortMode =
  'requests-desc' | 'amount-desc' | 'alpha-asc' | 'alpha-desc' | 'requests-asc' | 'amount-asc';
...
function sortMetricRows<T extends ProcureGuardAnalyticsMetric>(rows: T[], sortMode: SortMode): T[] {
  return [...rows].sort((a, b) => {
    ...
    if (sortMode === 'amount-asc')
      return a.amount - b.amount || a.count - b.count || a.label.localeCompare(b.label);
    if (sortMode === 'alpha-asc') return a.label.localeCompare(b.label);
    if (sortMode === 'alpha-desc') return b.label.localeCompare(a.label);
    if (sortMode === 'requests-asc')
      return a.count - b.count || a.amount - b.amount || a.label.localeCompare(b.label);
    return b.count - a.count || b.amount - a.amount || a.label.localeCompare(b.label);
  });
}

function sortHighValueRows(
  rows: ProcureGuardHighValueRequest[],
  sortMode: SortMode,
): ProcureGuardHighValueRequest[] {
  return [...rows].sort((a, b) => {
    if (sortMode === 'amount-asc')
      return a.amount_usd - b.amount_usd || a.vendor_name.localeCompare(b.vendor_name);
    ...
    <div className="min-h-[100dvh] bg-slate-50 p-4">
      <div className="mx-auto max-w-xl rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Sign in with an account that has ProcureGuard analytics access.
        </p>
        <Link
          href="/procure-guard"
...
function MetricCard({
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <div
        className={`inline-flex rounded-md border px-2 py-1 text-[0.6875rem] font-bold uppercase ${tones[tone]}`}
      >
        {label}
      </div>
...
function MetricList({
  ...
  title: string;
  rows: ProcureGuardAnalyticsMetric[];
  sortMode: SortMode;
  formatLabel?: (value: string) => string;
}) {
  const sortedRows = sortMetricRows(rows, sortMode);
  const useAmountBars = sortMode.startsWith('amount');
  const maxBarValue = sortedRows.reduce(
    (max, row) => Math.max(max, useAmountBars ? row.amount : row.count),
    0,
  );
  return (
    ...
    <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      ...
      {rows.length === 0 ? (
        ...
        <div className="grid min-h-52 grid-cols-2 items-end gap-3 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-12">
          {rows.map((row) => {
            const height =
              maxAmount > 0 ? Math.max(12, Math.round((row.total_amount / maxAmount) * 150)) : 12;
            return (
    ...
    <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-100 px-4 py-3">
        ...
        <p className="mt-1 text-xs text-slate-500">
          Current open requests grouped by review owner and current step, measured from the last
          status update.
        </p>
      </div>
      <div className="overflow-x-auto">
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.length === 0 ? (
              ...
              rows.map((row) => {
                ...
                return (
                  <tr
                    key={`${row.request_type}-${row.status}-${row.owner_label}`}
                    className="hover:bg-[#307c4c]/5"
                  >
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-[0.6875rem] font-semibold ${badge.className}`}
                      >
                        <span className={`h-1.5 w-1.5 rounded-full ${badge.dot}`} />
                        {badge.label}
                      </span>
                      <p className="mt-1 text-[0.6875rem] font-semibold uppercase text-slate-400">
                        {row.request_type}
                      </p>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right font-semibold text-slate-700">
                      {formatDuration(row.average_hours)}
                    </td>
                    <td className="px-4 py-3 text-right font-semibold text-slate-700">
                      {formatDuration(row.total_hours)}
                    </td>
                    <td className="px-4 py-3 text-right font-semibold text-amber-700">
                      {formatDuration(row.longest_hours)}
                    </td>
                    <td className="px-4 py-3">
                      <Link
                        href={href}
                      ...
                      >
                        {row.oldest_reference_number}
                      </Link>
                      <p className="mt-1 text-xs text-slate-500">
                        {row.oldest_vendor_name} | since {fmtDate(row.oldest_updated_at)}
                      </p>
                    </td>
                  </tr>
                );
    ...
    <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-100 px-4 py-3">
        <h2 className="text-sm font-bold text-slate-900">
          Highest Value Open Requests (USD Equivalent)
        </h2>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
                ...
                return (
                  <tr key={`${row.request_type}-${row.id}`} className="hover:bg-[#307c4c]/5">
                    <td className="px-4 py-3 font-bold text-slate-900">
                      <Link
                        href={`/procure-guard/${row.request_type === 'adhoc' ? 'adhoc-payments' : 'advance-payments'}/${row.id}`}
                    ...
                    <td className="px-4 py-3 text-right font-semibold">
                      ...
                      {row.currency !== 'USD' && (
                        <p className="text-[0.6875rem] font-normal text-slate-500">
                          {usdFmt(row.amount, row.currency)} original
                        </p>
                      )}
                    </td>
...
function ScopeTabs({
  ...
  value: RequestScope;
  onChange: (v: RequestScope) => void;
  counts: Record<RequestScope, number>;
}) {
  return (
    <div className="inline-flex rounded-lg border border-slate-200 bg-slate-50 p-1">
      {SCOPE_TABS.map((tab) => (
        ...
        >
          {tab.label}
          <span
            className={`ml-1.5 text-xs font-semibold ${value === tab.value ? 'text-white/80' : 'text-slate-400'}`}
          >
            {counts[tab.value]}
          </span>
        </button>
...
export default function AnalyticsClient({
  ...
  data: ProcureGuardAnalyticsData | null;
  embedded?: boolean;
}) {
  ...
  const reviewAging = useMemo(
    () =>
      (data?.review_duration_metrics ?? []).filter(
        (row) => scope === 'all' || row.request_type === scope,
      ),
    [data, scope],
  );
  ...
  const scopeNoun =
    scope === 'adhoc' ? 'adhoc PO' : scope === 'advance' ? 'advance payment' : 'request';
    ...
    <div className={`${embedded ? 'bg-white' : 'min-h-[100dvh] bg-slate-50'} text-slate-900`}>
      ...
      <main className={`${embedded ? '' : 'mx-auto max-w-[1320px] px-4 py-5'} space-y-4`}>
        {!embedded && (
          <ProcureGuardHero
            title="Analytics"
            subtitle="Spend, vendor, and approval insights across all ProcureGuard requests."
          />
        )}
        {embedded && (
          <section className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              <h2 className="text-lg font-bold text-slate-900 tracking-tight">
                ProcureGuard Payment Analytics
              </h2>
        ...
        {supplierQuery.trim() && (
          <p className="px-1 text-xs font-semibold text-slate-500">
            Showing {scoped.count} {scopeNoun}
            {scoped.count === 1 ? '' : 's'} for suppliers matching “{supplierQuery.trim()}”.
          </p>
        )}
        ...
        <section className="grid grid-cols-1 gap-3 md:grid-cols-3 xl:grid-cols-6">
          <MetricCard
            label="Total USD Eq."
            value={usdFmt(scoped.total_amount)}
            detail={`${scoped.count} ${scopeNoun}${scoped.count === 1 ? '' : 's'}`}
            tone="green"
          />
          <MetricCard
            label="Average USD Eq."
            value={usdFmt(scoped.average_amount)}
            detail="Mean value normalized to USD"
            tone="blue"
          />
          <MetricCard
            label="Vendors"
            value={scoped.vendor_count}
            detail="Unique vendors in view"
            tone="slate"
          />
          <MetricCard
            label="Requesters"
            value={scoped.requester_count}
            detail="Unique requesters in view"
            tone="slate"
          />
          <MetricCard
            label="Pending"
            value={scoped.pending}
            detail="Active approval chain items"
            tone="amber"
          />
          <MetricCard
            label="Top Vendor"
            value={scoped.top_vendor?.label ?? 'None'}
            detail={
              scoped.top_vendor
                ? `${scoped.top_vendor.count} ${scopeNoun}${scoped.top_vendor.count === 1 ? '' : 's'}`
                : 'No records'
            }
            tone="green"
          />
        ...
        <section className="flex flex-col gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Applies to vendor, status, requester, and open-request lists using USD-equivalent
              value.
            </p>
          </div>
        ...
        <section className="grid grid-cols-1 gap-4 xl:grid-cols-2">
          ...
          <MetricList
            ...
            rows={scoped.status_breakdown}
            sortMode={sortMode}
            formatLabel={formatProcureGuardStatusLabel}
          />
          <MetricList
            title="Requester Activity"
            rows={scoped.requester_breakdown}
            sortMode={sortMode}
          />
        </section>
