    ...
    <div className="min-h-[100dvh] flex items-center justify-center bg-slate-50 p-4">
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-10 text-center max-w-sm">
        ...
        <p className="text-sm text-slate-500">
          Check the ProcureGuard tables and database environment variables.
        </p>
      </div>
    </div>
  );
...
function PriorityPill({ priority }: { priority: string }) {
  return (
    <span
      className={`inline-flex px-2 py-0.5 rounded-full border text-[0.6875rem] font-semibold ${getPriorityBadge(priority)}`}
    >
      {priority}
    </span>
  );
...
function RequestRow({
  ...
  return (
    <Link
      href={href}
      className="group flex items-center justify-between gap-4 p-3 transition-colors hover:bg-[#307c4c]/5"
    >
      <div className="min-w-0">
        ...
        <p className="text-sm text-slate-600 truncate mt-0.5">
          {type} request · {request.vendor_name}
        </p>
        <p className="text-xs text-slate-400 mt-0.5">
          Created {fmtDate(request.created_at)} by{' '}
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <svg
          className="h-4 w-4 text-slate-300 transition-all group-hover:translate-x-0.5 group-hover:text-[#307c4c]"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={2}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#307c4c]/10 text-[#307c4c]">
          {icon}
        </div>
      </div>
      <p className="mt-1 text-2xl font-bold tracking-tight text-slate-900">{value}</p>
...
export default function ProcureGuardDashboardClient({
  data,
}: {
  data: ProcureGuardDashboardData | null;
}) {
      ...
      <main className="max-w-[1220px] mx-auto px-4 sm:px-6 py-4 space-y-4">
        <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#307c4c] to-[#1d4f31] p-4 sm:p-4 text-white shadow-lg shadow-[#307c4c]/25">
          ...
          <div className="relative flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
            <div>
              <p className="mb-1.5 text-[0.6875rem] font-semibold uppercase tracking-[0.3em] text-white/70">
                Guarding every purchase
              </p>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-0.5 max-w-2xl text-sm text-white/80">
                    Track adhoc POs and advance payment requests from submission through review and
                    approval.
                  </p>
                </div>
              </div>
            ...
            {data.actor.permissions.canCreateRequests && (
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => router.push('/procure-guard/adhoc-payments/new')}
                  className="rounded-lg bg-white px-4 py-2 text-sm font-bold text-[#1d4f31] shadow-sm transition hover:bg-white/90"
                >
                  New Adhoc PO
                </button>
                <button
                  onClick={() => router.push('/procure-guard/advance-payments/new')}
                  className="rounded-lg border border-white/30 bg-white/10 px-4 py-2 text-sm font-bold text-white backdrop-blur-sm transition hover:bg-white/20"
                >
                  New Advance Request
                </button>
              </div>
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
          ...
          <MetricCard
            ...
            sub="Completed approval requests"
            tone="green"
          />
          <StatCard
            label="Adhoc POs"
            ...
            icon={
              ...
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M3 7a2 2 0 012-2h14a2 2 0 012 2v10a2 2 0 01-2 2H5a2 2 0 01-2-2V7zm14 5h.01"
                />
              </svg>
            }
          />
          ...
          <div className="col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            {pendingQueue.length === 0 ? (
              ...
              <div className="divide-y divide-slate-100">
                {pendingQueue.map((r) => (
                  <RequestRow key={`${r._type}-${r.id}`} request={r} type={r._type} />
                ))}
              </div>
            )}
          </div>
                  ...
                  <div key={item.id} className="flex gap-3 p-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        ...
                        {item.on_behalf_of_name ? ` (on behalf of ${item.on_behalf_of_name})` : ''}
                      </p>
