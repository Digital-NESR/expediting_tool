    ...
    <div className="flex min-h-[100dvh] items-center justify-center bg-slate-50 p-4">
      <div className="max-w-sm rounded-lg border border-slate-200 bg-white p-4 text-center shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Check the local ProcureGuard database connection.
        </p>
      </div>
    </div>
  );
...
function PriorityPill({ priority }: { priority: string }) {
  return (
    <span
      className={`inline-flex rounded-full border px-2 py-0.5 text-[0.6875rem] font-semibold ${getPriorityBadge(priority)}`}
    >
      {priority}
    </span>
  );
...
function actionLabel(item: ProcureGuardWorkQueueItem) {
  ...
  if (item.actions.nextStatus)
    return `Open request to move to ${formatProcureGuardStatusLabel(item.actions.nextStatus)}`;
  ...
  return (
    <div className="min-h-[100dvh] bg-slate-50 font-sans text-slate-900">
      ...
      <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-gray-100 bg-white/80 px-4 backdrop-blur-md md:h-16 md:px-8">
        ...
        <Link
          href="/admin?tool=procureguard-access"
          className="ml-auto rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-[#307c4c]/5"
        >
          Manage Access
        </Link>
      </header>
            ...
            <div className="flex items-center gap-3.5">
              ...
              <div>
                ...
                <h1 className="mt-1 text-xl font-bold tracking-tight sm:text-2xl">
                  {data.actor.role} Work Queue
                </h1>
                <p className="mt-1 max-w-2xl text-sm text-white/80">
                  Open a payment to review the details, add comments, and complete your action.
                </p>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-2">
              <div className="rounded-lg border border-white/20 bg-white/10 px-4 py-3 text-center backdrop-blur-sm">
                <p className="text-[0.6875rem] text-white/70">Total</p>
                <p className="text-xl font-bold">{data.stats.total}</p>
              </div>
              <div className="rounded-lg border border-white/20 bg-white/10 px-4 py-3 text-center backdrop-blur-sm">
                <p className="text-[0.6875rem] text-white/70">Adhoc</p>
                <p className="text-xl font-bold">{data.stats.adhoc}</p>
              </div>
              <div className="rounded-lg border border-white/20 bg-white/10 px-4 py-3 text-center backdrop-blur-sm">
                <p className="text-[0.6875rem] text-white/70">Advance</p>
                <p className="text-xl font-bold">{data.stats.advance}</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
          {data.items.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Change your role in the admin permissions tab to test another approval queue.
              </p>
            </div>
          ) : (
                  ...
                  >
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-[0.6875rem] font-bold capitalize text-slate-600">
                          {item.request_type}
                        </span>
                      ...
                      <p className="mt-2 truncate text-sm font-semibold text-slate-900">
                        {item.request.vendor_name}
                      </p>
                      <p className="mt-1 text-xs text-slate-500">
                        {actionLabel(item)} | Current owner: {item.actions.ownerLabel} | Created{' '}
                        {fmtDate(item.request.created_at)}
                      </p>
                    </div>
                    <div className="flex flex-wrap items-center gap-3 lg:justify-end">
                      <div className="text-right">
                        <p className="font-bold text-slate-900">
                          {usdFmt(item.request.amount, item.request.currency)}
                        </p>
                        <p className="text-xs text-slate-400">{item.request.currency}</p>
                      </div>
                      <Link
                        href={href}
                        className="rounded-lg bg-[#307c4c] px-4 py-2 text-xs font-bold text-white hover:bg-[#307c4c]/80"
                      >
                        Open Review
                      </Link>
                    </div>
